...
@skip_without_clang
class Test原生类与方法代码生成:
    """A9/R9 收口：原生腿类代码生成修复的回归守单点（2026-08-27）。

    修的是三处环环相扣的 bug，全部是「能编出 exe 但一跑就崩/就错」：
    ① 方法体末尾无条件 `ret void` 造成双重终结符 → LLVM IR 验证红；
    ② 解析器把源码「己」归一成 Identifier('self')，而方法内槽位键是「己」，
       `self` 一路落到「当作字符串常量」→ dv_class_get_member/set_member
       拿字符串对象当对象，属性读写直接崩（0xC0000005）；
    ③ `己.方法名(...)` 被拍平成 SegmentName('self.方法名')，未路由到
       dv_call_method → 报「未定义的段落：self.xxx」。

    判据一律走生产路径 `compile_light_typed`（不自己拼 IR），真跑断言 stdout。
    """

    def test_普通类方法带参数(self):
        退出码, 输出行 = 走生产路径编译并运行(
            '类 盒子：\n  段落 放 接收 值：\n    返回 值\n  结束\n结束\n'
            '设 乙 为 新建 盒子。\n输出(乙.放(9))\n', 2, '普通类方法')
        assert 退出码 == 0, f'rc={退出码}'
        assert 输出行 == ['9'], f'普通类方法输出不符: {输出行}'

    def test_普通类构造加属性(self):
        退出码, 输出行 = 走生产路径编译并运行(
            '类 栈：\n  构造 接收 初值：\n    己.初值 为 初值。\n  结束\n'
            '  段落 取值：\n    返回 己.初值。\n  结束\n结束\n'
            '设 甲 为 新建 栈(7)。\n输出(甲.取值())\n', 2, '普通类构造')
        assert 退出码 == 0, f'rc={退出码}'
        assert 输出行 == ['7'], f'普通类构造输出不符: {输出行}'

    def test_泛型类构造加属性(self):
        """泛型类 `类 栈[T]` 带构造函数——类型擦除后与普通类同语义，必须真跑。"""
        退出码, 输出行 = 走生产路径编译并运行(
            '类 栈[T]：\n  构造 接收 初值：\n    己.初值 为 初值。\n  结束\n'
            '  段落 取值：\n    返回 己.初值。\n  结束\n结束\n'
            '设 甲 为 新建 栈[整数](7)。\n输出(甲.取值())\n', 2, '泛型类构造')
        assert 退出码 == 0, f'rc={退出码}'
        assert 输出行 == ['7'], f'泛型类构造输出不符: {输出行}'

    def test_泛型类方法无参(self):
        退出码, 输出行 = 走生产路径编译并运行(
            '类 盒子[T]：\n  段落 常量：\n    返回 3\n  结束\n结束\n'
            '设 乙 为 新建 盒子[整数]。\n输出(乙.常量())\n', 2, '泛型类方法')
        assert 退出码 == 0, f'rc={退出码}'
        assert 输出行 == ['3'], f'泛型类方法输出不符: {输出行}'

    def test_方法内调方法(self):
        """`己.方法名(...)` 必须路由到 dv_call_method，不许报「未定义的段落」。"""
        退出码, 输出行 = 走生产路径编译并运行(
            '类 栈：\n  段落 加一 接收 值：\n    返回 值 加 1\n  结束\n'
            '  段落 调：\n    返回 己.加一(5)\n  结束\n结束\n'
            '设 甲 为 新建 栈。\n输出(甲.调())\n', 2, '方法内调方法')
        assert 退出码 == 0, f'rc={退出码}'
        assert 输出行 == ['6'], f'方法内调方法输出不符: {输出行}'

    def test_方法内只写属性(self):
        """方法内写 `己.值 为 值` 并把更新后的对象写回「己」槽位。"""
        退出码, 输出行 = 走生产路径编译并运行(
            '类 盒子：\n  段落 放 接收 值：\n    己.值 为 值。\n    返回 值\n'
            '  结束\n结束\n设 乙 为 新建 盒子。\n输出(乙.放(9))\n', 2, '方法内只写')
        assert 退出码 == 0, f'rc={退出码}'
        assert 输出行 == ['9'], f'方法内只写输出不符: {输出行}'
