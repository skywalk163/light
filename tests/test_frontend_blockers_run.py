...
# ----------------------------------------------------------------------
# A9 泛型类实例化：`新建 类名[类型参数](构造参数)` 的类型参数不许混进构造实参
# ----------------------------------------------------------------------

def test_a9_泛型类构造参数不被类型参数吞掉():
    """`新建 栈[整数](7)`：`[整数]` 是类型参数，`(7)` 才是构造实参。

    改动前 v3 解析器把两者并成一个
    `FunctionCallExpr(callee=ListLiteral([整数]), args=[7])` 塞进 ClassInstantiation
    的 args——Python 转译腿编成 `栈([整数])(7)`（运行期 NameError: 整数 未定义），
    原生腿在表达式分派处报「暂不支持 FunctionCallExpr」。真跑两条腿都必须出 7。
    """
    _assert_stdout(
        '类 栈[T]：\n'
        '  构造 接收 初值：\n'
...
def test_a9_泛型类无构造参数():
    """`新建 栈[整数]`（无构造参数）：类型参数被吃掉后不能留下空壳 ListLiteral。"""
    _assert_stdout(
        '类 盒子[T]：\n'
        '  段落 常量：\n'
        '    返回 3\n'
