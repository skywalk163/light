...
@pytest.mark.xfail(strict=True, reason=(
    "A2-3 遗留缺陷：`等待 <局部变量>` 被静默编成一个名叫 `等待任务甲` 的标识符。"
    "词法层是对的（实测 tokenize 出 KEYWORD(等待) + IDENTIFIER(任务甲) 两个 token），"
    "所以粘连发生在 parser/codegen 的并置名拼接上。`等待 限时(...)`、`等待 取数()` "
    "能过是因为被调用名在词法保护表/定义名表里，掩盖了这个洞。"
    "strict=True：修好之后这条会以 XPASS 打红，逼下一手把 xfail 摘掉。"))
def test_a23_创建任务():
    """A2-3：`创建任务(协程)` → asyncio.create_task。

    产物侧已实测正确（`任务甲 = asyncio.create_task(报(3))`），卡在 `等待 任务甲`
    这一步的名字粘连上，不是 create_task 映射本身的问题。
    """
    _assert_stdout(
        '异步 段落 报(数)：\n'
        '    等待 异步睡眠(0.01)。\n'
...
def test_a23_创建任务映射到create_task():
    ...
    与上面那条 xfail 分开写，是为了让「映射对了」和「等待变量粘连」两件事各自
    有独立判据——否则 xfail 会把映射回归一起盖掉。
    """
    source = (
        '异步 段落 报(数)：\n'
        '    返回 数 乘 2。\n'
