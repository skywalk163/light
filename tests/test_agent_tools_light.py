...
class _假循环:
    ...
    # 第五参 副作用 在真实的 代理循环.注册工具 里有默认值 "写"（最保守）。
    # 这个假循环**故意不给默认值**：生产侧漏标只是退化成串行、不会报错，
    # 而测试侧漏标必须当场炸出来，否则「六个工具的副作用面」这件事就没人看着了。
    def 注册工具(self, 名称, 描述, 参数模式, 实现, 副作用):
        self.工具表[名称] = {
            "名称": 名称,
            "描述": 描述,
            "参数模式": 参数模式,
...
# 六个工具各自真实的副作用面。这张表是 A4 并行判据的输入，
# 标错的后果不是报错而是「本该串行的批被并行跑了」，所以必须钉死。
_期望副作用 = {
    "read_file": "只读",
    "list_dir": "只读",
    "grep": "只读",
...
def _wmic可用():
    ...
    处置口径：**Windows 上缺 wmic 就 skip，绝不放宽断言**——放宽「孙进程真死 /
    标记文件不出现」会把本用例变成恒真，正是本用例 docstring 明令禁止的。
    wmic 可用时用例照常真跑；**非 Windows（POSIX 杀树走 os.killpg，不需要 wmic）
    一律返回真**，避免在 Linux/FreeBSD 上白白丢掉这条覆盖。
    """
    global _WMIC_OK
...
class Test纯光明实现:
    ...
    def test_注册全部签名(self):
        ...
        # （原为 `段落 注册全部 接收 循环, 根目录, 选项`）。
        assert "段落 注册全部(循环, 根目录, 选项)" in 内容
...
# ============================================================
# 合并期补：六个工具的 副作用 第五参（A4 并行判据的输入）
# ============================================================
class Test副作用标注:
    """B4 按任务书 §7 把第五参留到合并期补。补完必须有人看着：
    标错的后果不是报错，而是「本该串行的批被并行跑了」——静默的正确性问题。"""

    def test_六个工具的副作用与真实副作用面一致(self, tmp_path):
        循环, 工具 = _装备(str(tmp_path))
        实际 = {名: 项["副作用"] for 名, 项 in 工具.items()}
        assert 实际 == _期望副作用, f"副作用标注与预期不符：{实际}"

    def test_只读批可并行而含写或执行的批不可(self, tmp_path):
        """不止断字典，还要断调度层真按它走：直接问 代理循环.可整批并行。

        这比「工具表里字段对不对」强一层——字段标对了但判据没读它，这条会红。"""
        from 代理循环 import 代理循环 as _真循环
        真 = _真循环("http://127.0.0.1:1", "test-model", "",
                   最大轮数值=3, 消息上限值=20)
        注册全部(真, str(tmp_path), {})

        def _计划(*名字们):
            return [{"名称": n, "副作用": 真.工具表[n]["副作用"],
                     "状态": "可执行", "文本": "", "调用id": "c%d" % i}
                    for i, n in enumerate(名字们)]

        assert 真.可整批并行(_计划("read_file", "grep", "list_dir")) is True, \
            "三个只读工具应判为可并行"
        assert 真.可整批并行(_计划("read_file", "write_file")) is False, \
            "含写工具的批不许并行"
        assert 真.可整批并行(_计划("read_file", "run_command")) is False, \
            "含执行工具的批不许并行"
        assert 真.可整批并行(_计划("read_file")) is False, \
            "单项批不该起线程"
