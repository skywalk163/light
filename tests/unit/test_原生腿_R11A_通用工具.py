...
"""
    ...
    # ───────────────── 跨模块导入已定义段落（T2 回归） ─────────────────
    def test_跨模块导入_已定义段落(self):
        # T2 回归：原生后端必须支持「从其他模块导入已定义段落并调用」
        # （完全匹配 类场景：数据验证.light:15 从 正则表达式 导入 完全匹配，
        #  正则表达式.light:44 将其定义为段落）。R83 任务书曾误判为 codegen
        #  跨模块导入解析不支持，但实测端到端可用——此用例锁死该行为，防止
        #  原生后端在后续改动中退化回「跨模块导入已定义段落」不可调用。
        # 同时覆盖：数据验证.light 内部又从 正则表达式 导入 完全匹配（传递依赖），
        # 主入口直接从 数据验证 导入 验证必填/验证类型 并调用。
        src = """从 正则表达式 导入 完全匹配
从 数据验证 导入 验证必填 验证类型 是否有效
段落 主:
  输出("M_OK=" 加上 转文本(完全匹配("^\\\\d+$", "12345")))
  输出("M_BAD=" 加上 转文本(完全匹配("^\\\\d+$", "abc")))
  设 r 为 验证必填("x", "name")
  输出("VR_OK=" 加上 转文本(是否有效(r)))
  设 r2 为 验证必填("", "name")
  输出("VR_BAD=" 加上 转文本(是否有效(r2)))
  设 r3 为 验证类型(5, "int", "n")
  输出("VT_OK=" 加上 转文本(是否有效(r3)))
"""
        got = _native_run(src)
        # 完全匹配：跨模块导入的已定义段落，直接调用
        self.assertEqual(got["M_OK"], "真")
        self.assertEqual(got["M_BAD"], "假")
        # 数据验证 的已定义段落：跨模块调用
        self.assertEqual(got["VR_OK"], "真")
        self.assertEqual(got["VR_BAD"], "假")
        self.assertEqual(got["VT_OK"], "真")
