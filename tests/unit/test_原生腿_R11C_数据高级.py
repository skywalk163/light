...
class Test农历:
    ...
    def test_O0_端午_对拍(self):
        """2024-06-10 = 农历五月初五（端午节，真实日历；R13B 起以精确数据为准）"""
        code = (
            '从 农历 导入 农历年 农历月 农历日\n'
            '段落 主:\n'
            '  输出(农历年(2024, 6, 10))\n'
        ...
        _断言对拍(code, [2024, 5, 5], '端午')

    def test_O0_闰月判断_对拍(self):
        """2025年闰六月，2024年无闰六月，2028年闰五月（真实日历，R13B 起以精确数据为准）"""
        code = (
            '从 农历 导入 判断闰月\n'
            '段落 主:\n'
            '  输出(判断闰月(2025, 6))\n'
        ...
        _断言对拍(code, [True, False, True], '闰月判断')

    def test_O0_农历转公历_对拍(self):
        """农历2024年五月初五 = 公历2024-06-10（端午，真实日历）"""
        code = (
            '从 农历 导入 农历转公历\n'
            '段落 主:\n'
        ...
        # 2024端午=6月10日，2024中秋=9月17日（真实日历；R13B 起 .light 用精确
        # 农历数据，.py 数据表的 2024 月长与实际日历差 1 天，以精确数据为准）
        _断言对拍(code, [2024, 6, 10, 2024, 9, 17], '农历转公历')
...
class Test中国传统节日:
    ...
    def test_O0_农历节日精确日期(self):
        """农历节日用精确农历数据表（2024端午=6月10日，中秋=9月17日，真实日历）"""
        code = (
            '从 中国传统节日 导入 获取节日日期\n'
            '段落 主:\n'
        ...
        # 农历节日用精确农历数据表（2024端午=6月10日，中秋=9月17日，真实日历；
        # .py 数据表月长偏差导致其给出 06-11/09-18，R13B 起以精确数据为准）
        _断言对拍(code, ["2024-06-10", "2024-09-17"], '农历节日精确日期')
...
class Test中国行政区划:
    ...
    def test_O0_城市列表_对拍(self):
        ...
        # R13B 起为完整城市列表（21 个，与 .py 一致）
        _断言对拍(code, [
            len(cities),
            cities[0],
            cities[-1],
            r.validate_city("广东省", "广州市"),
            r.validate_city("广东省", "云浮市"),
...
class Test网络请求:
    ...
    def test_O0_URL编码解码(self):
        ...
        # 解码URL: "hello+world" → "hello+world"（+ 不转空格，与 .py unquote 一致）
        # 解码URL: "a%3Fb" → "a?b"（R13B 起支持 %XX 十六进制解码，与 .py unquote 对齐）
        _断言对拍(code, ["hello", "hello+world", "a?b"], 'URL编码解码')

    def test_O0_POST_能力已扩展(self):
        """R13B 起 POST 已支持（不再返回「仅支持 GET」错误）。
        此处用不可达端口做离线验证：连接失败而非方法拒绝。"""
        code = (
            '从 网络请求 导入 发起请求 响应状态码\n'
            '段落 主:\n'
            '  设 r 为 发起请求("http://127.0.0.1:1/offline", "POST", 空, 空, 2)\n'
            '  输出(响应状态码(r))\n'
            '  输出(r["错误"] 等于 "仅支持 GET 方法")\n'
        )
        _断言对拍(code, [0, False], 'POST-能力已扩展')

    def test_O0_HTTPS_能力已扩展(self):
        """R13B 起 HTTPS 已接入 runtime TLS（不再返回「不支持 HTTPS」错误）。
        此处用不可达端口做离线验证：连接失败而非协议拒绝。"""
        code = (
            '从 网络请求 导入 发起请求 响应状态码\n'
            '段落 主:\n'
            '  设 r 为 发起请求("https://127.0.0.1:1/offline", "GET", 空, 2)\n'
            '  输出(响应状态码(r))\n'
            '  输出(r["错误"] 等于 "仅支持 HTTP（不支持 HTTPS）")\n'
        )
        _断言对拍(code, [0, False], 'HTTPS-能力已扩展')
