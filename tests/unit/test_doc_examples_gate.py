...
等于没有基线）。六类各自的含义与修法：
    ...
    ROT           像真代码但编不过，是真文档腐烂（大宗曾是已移除语法
                  `定义 x 等于 y`）。**这才是本门禁逐条钉住的目标。**
    ANNOTATED     真代码里夹了行内箭头注解（`← 推断为 数|空`）。
                  修法是把注解挪进注释，不是改代码。
    ESCAPED       引号被反斜杠转义污染。
    PSEUDO        一个光明关键字都没有，纯伪代码/签名说明。
    REPL          `>>>` 会话记录，天然编不过。
    COMPILER_BUG  编译器自己炸了，**与文档对错无关**，见下。

后四类的修法都是「改文档体裁/标签」，不是「改代码」，所以门禁对它们只卡总数
不卡逐条；ROT 才逐条钉 `(file, hash)`。

⚠️ **各类的条数不在这里复述——`tests/unit/doc_examples_baseline.json` 是唯一真值。**
（第七轮 E7 订正：这段原来写着「ROT 238 / ANNOTATED 18 / … 共 305」的首轮快照，
而基线早已不是那个分布了。把数字抄进 docstring 就等于又造一个会过期的第二真值源，
下一次基线刷新后它照样说谎，而且没有任何门禁会发现。要看当前分布就读基线文件，
或者跑一次本文件的门禁用例——它是照基线判的。）
