...
# L-072（方案A）改口径：转字符串 不再是裸 str()，Python 的 repr 不许漏进光明。
#   * True/False/None → "真"/"假"/"空"（原为 "True"/"False"/"None"）
#   * dict/list        → JSON 文本，双引号 + ensure_ascii=False（原为单引号的 Python repr）
#   * str/数字/其余类型 → 保持 str() 原行为不变
# tuple **刻意不改**：JSON 没有元组，方案A 只点名 字典/列表，元组继续走 str()。
# 表里前 4 条带 ← L-072 注释的即本轮改判的断言。
_转字符串表 = [
    ("None", None, "空"),                       # ← L-072：原 "None"
    ("布尔真", True, "真"),                     # ← L-072：原 "True"
    ("布尔假", False, "假"),                    # ← L-072：原 "False"
    ("整数", 42, "42"),
    ("浮点整值", 1.0, "1.0"),
    ("浮点", 1.5, "1.5"),
    ("负零浮点", -0.0, "-0.0"),
    ...
    ("列表", [1, 2], "[1, 2]"),                 # JSON 与 repr 在此恰好同形
    ("嵌套字典", {"k": 1}, '{"k": 1}'),          # ← L-072：原 "{'k': 1}"
    ("空字典", {}, "{}"),
    ("元组", (1, "a"), "(1, 'a')"),              # 元组不入 JSON 分支，仍是 repr
    ("自定义__str__", _自定义文本(), "自定义的字符串形态"),
    ("自定义__str__非ASCII", _文本形态怪(), "／／非ASCII／／"),
]


# 方案A 的容器分支判据：中文不转义、双引号、紧凑无缩进、嵌套递归。
# 上表只能覆盖单层小字典，这几条把 L-072 的规格钉全。
_转字符串容器表 = [
    ("中文键值不转义", {"名字": "张三", "年龄": 25},
     '{"名字": "张三", "年龄": 25}'),
    ("字典套列表", {"名字": "张三", "标签": ["开发者", "测试"]},
     '{"名字": "张三", "标签": ["开发者", "测试"]}'),
    ("三层嵌套", {"外": {"内": {"底": [1, ["二层"]]}}},
     '{"外": {"内": {"底": [1, ["二层"]]}}}'),
    ("列表套字典", [{"a": 1}, {"b": 2}], '[{"a": 1}, {"b": 2}]'),
    # 容器**内部**的布尔/空走 JSON 口径（true/false/null），
    # 只有顶层标量才发光明字面 真/假/空——两套口径不许串味
    ("容器内布尔与空", {"开": True, "关": False, "无": None},
     '{"开": true, "关": false, "无": null}'),
    ("列表内布尔与空", [True, False, None], "[true, false, null]"),
    # 容器内的字符串必须带引号，与顶层裸串形成对照
    ("容器内字符串带引号", ["abc"], '["abc"]'),
]


@pytest.mark.parametrize("名, 入参, 期望", _转字符串容器表,
                         ids=[c[0] for c in _转字符串容器表])
def test_转字符串_容器发JSON(名, 入参, 期望):
    光 = 光明转换.转字符串(入参)
    原 = 地板.转字符串(入参)
    assert 光 == 期望, "转字符串(%r) 应得 %r，实得 %r" % (入参, 期望, 光)
    assert "\\u" not in 光, "中文被转义成了 \\uXXXX（ensure_ascii 没关）"
    assert 光 == 原, "光明版与 Python 地板不一致：%r vs %r" % (光, 原)


def test_转字符串_顶层布尔空与容器内口径不同():
    """同一个 True，顶层发 "真"、容器内发 "true"——这是方案A 刻意的两套口径。

    顶层是给人看的光明字面；容器内是 JSON 文本，必须合 JSON 语法。
    """
    assert 光明转换.转字符串(True) == "真"
    assert 光明转换.转字符串([True]) == "[true]"
    assert 光明转换.转字符串(None) == "空"
    assert 光明转换.转字符串([None]) == "[null]"
...
def test_转字符串_标量不吞异常():
    """标量走的仍是裸 str()：__str__ 抛错必须原样冒出来，不许被 L-072 的 try 兜住。

    L-072 只给**容器分支**加了 try（序列化失败退回 str），标量分支一行没动。
    这条守住那个边界——否则「转字符串 从不抛错」会把调用方的真实故障静默掉。
    """
    class 会炸的:
        def __str__(self):
...
def test_转字符串_容器序列化失败退回str而不抛():
    """容器里塞了不可 JSON 化的值时，退回 str() 而不是抛错。

    转字符串 大量出现在错误消息拼接与日志输出里（stdlib/断言工具轻量.light:22、
    stdlib/代理循环.light:766 等）。它自己再抛错会把原始故障整个盖掉，
    所以容器分支刻意兜一层。
    """
    class 不可序列化:
        def __repr__(self):
    ...
    光 = 光明转换.转字符串({"k": 不可序列化()})
    原 = 地板.转字符串({"k": 不可序列化()})
    assert 光 == "{'k': <不可序列化>}", "应退回 Python 的 str()，实得 %r" % (光,)
    assert 光 == 原
