...
被测产物：stdlib/HTTP服务端.light（纯光明实现，socket 底座 + E9 并发原语）。
...
#
# 两条都已在 stdlib/HTTP服务端.light 修掉（H1 段落 置非阻塞、H2 异步接受 的
...
def test_accept可恢复错误不终止监听():
    ...
    反跑点：删掉 stdlib/HTTP服务端.light 异步接受 里
...
def test_停机原因区分正常停机与致命错误():
    ...
    反跑点：删掉 stdlib/HTTP服务端.light 异步接受 末尾的
...
def test_每条连接的选择器都被关掉():
    ...
    反跑点：删掉 stdlib/HTTP服务端.light 处理连接 的 最终 里那三行
