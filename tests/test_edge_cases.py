...
"""
        ...
        # 路4真除裁决（取代 09-04 裁决 B §15.1）：「除」改为 Python 真除（对齐原生腿
        # T7A），`10 除 2` 得浮点 5.0 → 3*4+5.0 = 17.0。审计锁定的优先级结论不变，
        # 仅结果类型由整型 17 变回浮点 17.0。
        # 此处用精确断言锁定真实行为，防止再被"改期望值"蒙混过关。
        # 标准数学优先级: 3 * 4 + 10 / 2 = 12 + 5.0 = 17.0（路4真除下为浮点）
...
"""
        ...
        # 路4真除裁决（取代 09-04 裁决 B §15.1）：「除」改为 Python 真除（对齐原生腿
        # T7A），`4 除 2` 得浮点 2.0 → 1 + 6 - 2.0 = 5.0。审计锁定的优先级结论不变，
        # 仅结果类型由整型 5 变回浮点 5.0。
        # 精确断言锁定真实行为。标准数学优先级: 1 + 6 - 2.0 = 5.0（路4真除下为浮点）
