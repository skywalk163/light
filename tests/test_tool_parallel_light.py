...
class Test并行时序:
    ...
    def test_并发上限真的限制了同时在跑的数量(self):
        ...
        # 上界锚在「串行下界」而不是写死 0.9：CI 是 FreeBSD + pytest -n auto，
        # 四个任务分两批在高负载下越过 0.9 完全可能，而那不是回归。
        # 串行会是 4×睡 = 1.2s，所以 4×睡-0.15 = 1.05 仍然能把「退化成串行」判红，
        # 同时给两批的正常抖动留出余量。下界 0.5 继续把「上限是摆设、一次全起（≈0.3s）」判红。
        串行下界 = 4 * 睡 - 0.15
        assert 0.5 < 耗时 < 串行下界, (
            "并发上限=2 跑四任务耗时 %.3fs，不在「两批」区间 (0.5, %.2f)" % (耗时, 串行下界))
...
class Test失败隔离:
    ...
    def test_校验失败的项自己不执行但不拖垮同批只读项(self):
        """参数不合 schema 的项在编排阶段就被判失败，实现不会被调用。

        名字按实际行为写：这一批两项都标「只读」，所以它**走的是并行路径**，
        不是串行——原名「让整批走串行」与代码不符，且本用例也没有断串行。
        （2026-08-23 合并期改名，断言未动。）"""
        a = _agent()
        进过 = []
