...
> **术语澄清（2026-08-29）**：L0–L7 是**语法层级（文体与方言）**，
> **不是**编译器内部分层（词法/语法/编译）。编译器分层见
> [docs/architecture.md](docs/architecture.md)：5 段前端 + 代码生成 + 三后端。
> 本文下方「v4.0 五层语法架构」讲的是 L0–L4 这一组教学与工程分级，
...
| LLVM 后端 | ⚠️ 部分 | 支持编译为原生 EXE（clang 零错误），但**不支持依赖 Python 生态的项目**（`导入 asyncio` 等会报模块未找到）。能力边界见 [docs/原生腿能力边界.md](docs/原生腿能力边界.md) |
