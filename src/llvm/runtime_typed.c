...
void dv_list_reverse(LightValue* result, LightValue* list) {
     ...
     * dv_list_reverse，字符串链式调用返回空列表（实为分派缺失）。 */
    if (list->type == 3) {
...
void dv_to_int(LightValue* result, LightValue* v) {
    ...
    if (v->type == 5) {
         ...
         * bool dv 的真值在 boolean 字段（i64 字段恒 0）。 */
        result->type = 1;
