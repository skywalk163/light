...
_CHINESE_HINTS = {
    'SyntaxError': '请检查代码语法是否正确，确保所有括号、引号、冒号、句号等符号已正确配对。光明语句需要以句号「。」结尾，代码块以冒号「：」开始。',
    'TypeError': '请检查操作数类型是否匹配，光明中文本和数字不能直接进行运算。可使用「整数()」「文本()」等函数进行类型转换。',
    'ValueError': '请检查传入的值是否在有效范围内，可能需要先进行类型转换或做边界检查。',
    'NameError': '请检查变量名是否拼写正确，使用前需先通过「设」关键字定义变量。光明中变量必须先定义后使用。',
    'IndexError': '请检查索引是否在有效范围内，光明列表索引从 0 开始，长度为 N 的列表最大索引为 N-1。可使用「长度()」获取列表长度。',
    'KeyError': '请检查字典键是否存在，可以使用「字典包含键()」方法先判断键是否存在，或用「字典获取()」提供默认值。',
    'AttributeError': '请检查对象是否拥有该属性或方法，需确认类定义中已声明。注意区分属性和方法——方法需要加括号调用。',
    'ImportError': '请检查模块名是否拼写正确，确认模块已安装或在标准库路径中。光明中模块名使用中文，如「导入 数学」。',
    ...
    'IndentationError': '缩进不正确，请检查代码缩进是否一致。光明使用 4 个空格作为标准缩进，不要混用 Tab 和空格。',
    ...
    'ModuleNotFoundError': '模块未找到，请检查模块名是否拼写正确，确认模块已安装。光明中可使用「导入 模块名」导入模块。',
    ...
    'SystemError': '解释器内部错误，请记录错误信息并报告给光明开发团队。',
    # 光明自定义错误提示
    'LexerError': '词法分析时发现非法字符或无法识别的符号。请检查代码中是否包含光明不支持的字符，确保使用正确的中文关键字。',
    'ParserError': '语法解析失败，请检查代码结构是否正确。光明中语句通常以句号「。」结尾，段落定义以冒号「：」结尾。',
    'SemanticError': '语义分析错误，请检查变量是否已定义、类型是否匹配、函数调用参数是否正确。光明要求先定义后使用。',
    'CodeGenError': '代码生成失败，请检查代码中是否存在光明不支持的语法结构或特殊用法。',
    'CompileError': '编译过程中出现错误，请根据具体错误信息调整代码。可尝试使用「--backend」切换编译后端。',
    'DuanError': '光明运行时错误，请根据具体错误信息进行排查和修复。',
...
_EXAMPLE_SNIPPETS = {
    ...
    'CodeGenError': '  请检查代码中是否存在光明不支持的语法结构',
