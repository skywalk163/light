...
IDENTIFIER_SAFE_KEYWORDS = frozenset({
    # ── 第22轮 任务2 精简记录（2026-09-14）──────────────────────────
    # 逐条隔离中立验证（全语料 763 个 .light 文件 token 序列对比 + 单测回归，
    # 判据③）：17 条中 12 条冗余（包含/匹配/回调/外部/排序/接口/枚举/
    # 结构体/联合体/段落/返回/配），撤掉后全语料 token 零变化，删除。
    # 保留 5 条真护栏：
    ...
    #   打印 ← light-merge/examples/L2_wenyan/主程序.light、学生模块.light、
    #          light-merge/tests/test_iface.light
    #   标准库 ← 语料级中立但单测契约
    #     （tests/unit/test_lexer.py::test_identifier_safe_module_print_merged_inside_word
    #      断言 `我的标准库` 合并为标识符），保留。
    # 证据：lightharness/_task2_R22_IDENTIFIER_SAFE精简_交付报告.md
    '函数',   # 护栏（R22任务2）
    '输出',   # 护栏（R22任务2）
    '模块',   # 护栏（R22任务2）
    '打印',   # 护栏（R22任务2）
...
IDENTIFIER_SAFE_SUFFIX_ONLY_KEYWORDS = frozenset({
    # ── 第22轮 任务2 精简记录（2026-09-14）──────────────────────────
    # 标准库：语料级隔离中立（全语料 763 文件 token 零变化），但单测
    # tests/unit/test_lexer.py::test_identifier_safe_module_print_merged_inside_word
    # 断言 `我的标准库` 合并为标识符（行为契约），故**保留**、不删。
    '标准库', '模块', '打印',  # 护栏（R22任务2，依赖文件见 IDENTIFIER_SAFE_KEYWORDS 上方 R22 记录）
})
...
COMMON_COMPOUND_WORDS = frozenset({
    # ── 第22轮 任务4 精简记录（2026-09-14）──────────────────────────
    # R21 保留的 32 条 TierB 护栏（压缩模量/去重行/各乘数据/各减数据/各加数据/
    # 各除数据/弹性模量/当前含/当前含水/当前环节/当前环节数/当家/当选/承载力/
    # 承载能力/文本去重行/格式模式/清除率/清除速率/滚动乘积/滚动加权/滚动求和/
    # 生成分析阶段/甲位与乙/终期折现因子/联合体成员/跳过迭代/过滤模二零/
    # 过滤模二非零/过滤模十零/过滤模零/过滤模零集合）经逐条隔离中立验证
    # （全语料 763 文件 token 零变化）已全部冗余——第21轮上下文敏感切词覆盖
    # 了它们的保护场景，32 条全部删除（216→184 条）。
    # 证据：lightharness/_task4_R22_TRAILING_ALIAS+CCW护栏精简_交付报告.md
...
# ── 第22轮 任务4 精简记录（2026-09-14）──────────────────────────────
# 逐条隔离中立验证（全语料 763 文件 token 序列零变化）：遍/捕/抛/终/返/设/承/
# 宏/掷 9 条冗余（已被第21轮上下文敏感切词覆盖），删除。保留 1 条真护栏：
#   己 ← lightharness/examples/test_L120.light（L-120 复现用例）
# 证据：lightharness/_task4_R22_TRAILING_ALIAS+CCW护栏精简_交付报告.md
_TRAILING_ALIAS_MERGE = frozenset({
    '己',  # 护栏（R22任务4，依赖文件：lightharness/examples/test_L120.light）
})
