...
_COMPOUND_SAFE_SINGLE_KEYWORDS = frozenset({
    ...
    # 同族的 `约` 本单**有意未加**（也未进 keywords.py）：`约` 进表后，
    # `合约乘数` 这类「约 紧跟另一个 compound-safe 单字」的串会踩到本函数
    # 「回报内层结果」的返回值失配（见 _skip_compound_safe_and_match 文档串），
    # 实测编成 `合 * 乘数`——源文本里的 `约` 整字消失。详见工单 31-B。
