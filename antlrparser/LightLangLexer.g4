...
// 范围表达式（1至10 / 1到10步2）——R76-A：自定义分词器将 至/到/步 映射到此处
K_TO       : '至' | '到' ;
K_STEP     : '步' ;

// 访问控制修饰符（公有/私有/保护）——R76-A
K_PUBLIC   : '公有' ;
K_PRIVATE  : '私有' ;
K_PROTECTED: '保护' ;
K_STATIC   : '静态' ;

// 成员关系运算符（'.' 在 列表）——R76-A（K_PARENT 已在下方类相关区定义）
K_IN       : '在' ;
...
// 缺陷：`己结果 等于 己结果 加 x` 会被解析成
//   assignStmt(己 结果 等于 (primary 己)) + exprStmt(结果 加 x)
// —— 因为 primary 的 K_SELF 备选先匹配，尾随 ID 被拆成下一条语句。
// 让 `己`/`自`/`自我` 紧跟汉字标识符时整体成 token，消除该歧义。
// 注：该规则必须排在 K_SELF 之前才能生效（ANTLR 最长匹配优先，同长取先出现者）。
K_SELF_PROP: ('己' | '自我' | '自') [\u4e00-\u9fffA-Za-z_] [\u4e00-\u9fff0-9A-Za-z_]* ;
K_METHOD   : '方法' ;
