...
2. **后端取值正向覆盖**：从 `cli/light.py` 源码里抓出所有 `--backend` 的 `choices`
   ...
   摘掉后 `坏` 计数下降是真进步，不该被拦。
   摘除后的收尾口径（B9 S2 定）：那一条整条挪进清单的 `已摘除CLI路径` 归档区
   （本门禁不读这个键），**不是**只把 `状态` 改成 `未实现` —— `后端取值` 是从
   `命令` 串里抽的，与状态词无关，只改状态告警照旧。归档不是豁免名单：
   `tests/unit/test_ci_gates_round9.py::Test真清单在册` 断言归档里的取值不许仍在
   `cli/light.py` 的 `choices` 里（想把一条活着的坏路径挪进归档躲 `坏` 计数 → 红），
   而「表里有、源码已无」这件事本身在真仓库上也被那个类断成红，不再只是一行告警。
...
def main():
    ...
    if 统计["backend_已摘除"]:
        print("[原生产品门禁] 告警（不判红）：CLI路径表登记了源码里已无的后端取值：%s"
              "（摘除死腿是许可处置；把表里那条整条挪进清单的 `已摘除CLI路径` 归档区"
              "即可清掉本告警 —— 只改 `状态` 消不掉：`后端取值` 是从 `命令` 串里抽的，"
              "与状态词无关）"
              % "、".join(统计["backend_已摘除"]))
