...
def main():
    ...
    # D4-4：`--dry-run` 已删除。它打的是硬编码字面量（"105 条违规"真值 103、
    # "run #66 492.6s"），从未被任何 workflow 调用，唯一出口 return 0——
    # 作为「确认新增没吃掉预算」的证据被引用过（第三轮留档/D3交付报告.md、
    # 合并报告_第三轮.md），但硬编码会腐坏，引用即误导。要真判定请看 --check。
    # 两份文档已加勘误。删除理由全文见 第四轮留档/。
    args = ap.parse_args()
