...
从 __index__.py 元数据自动生成 lightpub 桥接模块骨架。
根据 lightpub 包名 → Python stdlib 映射，为每个包生成等价的 Python 桥接模块。

用法:
    python tools/lightpub_bridge.py --list-available
    python tools/lightpub_bridge.py --package 文件系统
    python tools/lightpub_bridge.py --batch P0
    python tools/lightpub_bridge.py --batch 5
    python tools/lightpub_bridge.py --package 文件系统 --with-tests
    python tools/lightpub_bridge.py --package 文件系统 --dry-run
"""
...
# stdlib 中的 lightpub 桥接模块目录
_STDLIB_LIGHTPUB_DIR = _PROJECT_ROOT / 'stdlib' / 'lightpub'

# __index__.py 路径
_INDEX_PATH = _STDLIB_LIGHTPUB_DIR / '__index__.py'

# __init__.py 路径
_INIT_PATH = _STDLIB_LIGHTPUB_DIR / '__init__.py'

# 测试目录
_TESTS_DIR = _PROJECT_ROOT / 'tests' / 'lightpub'
...
def load_index():
    ...
    sys.path.insert(0, str(_STDLIB_LIGHTPUB_DIR.parent))
    try:
        # 动态导入 __index__
        ...
        if str(_STDLIB_LIGHTPUB_DIR.parent) in sys.path:
            sys.path.remove(str(_STDLIB_LIGHTPUB_DIR.parent))
...
def generate_test_content(pkg_name, pkg_info):
    ...
    lines.append(f'')
    lines.append(f'from lightpub.{pkg_name} import *')
    lines.append(f'')
...
def update_init_bridge_map(new_bridges):
    ...
    indent = '    '
    dict_lines = [f'{indent}# P0 包名 → lightpub 桥接模块名映射']
    dict_lines.append(f'{indent}# 这些包已有 Python 桥接实现，导入时路由到 stdlib/lightpub/ 下的桥接模块')
    ...
    for name, mod in sorted_items:
        dict_lines.append(f"{indent}{indent}'{name}':\t'{mod}',\t\t# 桥接: stdlib/lightpub/{mod}.py")
...
示例:
  ...
  %(prog)s --package 文件系统 --force           # 覆盖已存在的桥接模块
    ...
    parser.add_argument('--output-dir', type=str, default=str(_STDLIB_LIGHTPUB_DIR),
                        help=f'输出目录（默认: {_STDLIB_LIGHTPUB_DIR}）')
