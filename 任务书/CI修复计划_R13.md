...
> 生成：2026-09-07。状态：**任务 2 / 任务 4 已修复**（CI-import-hook-CIB 分支完成，整进程定向 65 passed / 0 failed）。
...
## 任务 5：对象池缓存 O0 codegen 回归

**根因（现象已本地复现，引入点待二分）**：`test_对象池缓存_冒烟_O0` 期望 `对象池放入("a")/("b")` 后取回 `'b'/'a'`，实际取回 `'[]'/'空'`（槽默认值）——**放入的值没写进池槽**。`对象池缓存.light` 是 R12 期间 .light 化的；嫌疑在 R12B「嵌套容器写回」或 R12C「槽位池」改动改变了池内部的列表/字典写回语义。

**步骤**：
1. 最小化复现：`创建对象池 → 对象池放入(池,"a") → 对象池获取(池)` 三行，LLVM O0 编译，dump 中间产物看写入路径。
2. 用 `git bisect`（或手工切 `39d22ab9` 前后）定位引入 commit。
3. 修 codegen（预期在 `src/llvm/codegen_typed.py` 写回/槽位相关段），把最小用例固化为回归测试。

**验收**：`test_对象池缓存_冒烟_O0` 绿；`ci_eval` 冒烟可运行率回到 1.0。

**改动面**：`src/llvm/codegen_typed.py`（与并行任务冲突风险最高的一处，建议与并行任务负责人对齐窗口）。
