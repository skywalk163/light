...
def 等待进程(进程: subprocess.Popen, 超时: float = None) -> 命令结果:
    ...
    try:
        ...
        #
        # ⚠️ 只 kill() 不回收是不够的（L-167）：kill 之后进程进入「已终止但
        #   未回收」状态——POSIX 上成为僵尸占着 PID 表项，Windows 上管道
        #   读线程仍在跑且 Popen 析构时抛 ResourceWarning；全量回归连跑数百
        #   个用例时残留累积，抬高系统空闲负载，进而让后续用例的进程启动
        #   变慢。故 kill 后再 communicate() 一次：既回收僵尸，也排空管道。
        进程.kill()
        try:
            进程.communicate()
