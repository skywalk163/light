# Generated by Light bootstrap compiler
import sys
import types
...
def parse_or_expr(tokens, pos):
    ...
    while (p < _light_builtin.列表长度(tokens)):
        ...
        if ((ttype == '关键字') and (tval == '或')):
            ...
            ight = _light_builtin.列表获取(ight_result, 0)
            p = _light_builtin.列表获取(ight_result, 1)
...
def parse_primary(tokens, pos):
    ...
    if (ttype == '标识符'):
        ...
        if (p < _light_builtin.列表长度(tokens)):
            ...
            if (_light_builtin.字典获取(next_t, '种别') == '左括号'):
                ...
                p += 1
                if (p < _light_builtin.列表长度(tokens)):
                    p_tok = _light_builtin.列表获取(tokens, p)
                    if (_light_builtin.字典获取(p_tok, '种别') != '右括号'):
                        while True:
                            ...
                            break
                if (p < _light_builtin.列表长度(tokens)):
                    p = _light_builtin.列表获取(tokens, p)
                    if (_light_builtin.字典获取(p, '种别') == '右括号'):
                        p += 1
    ...
    if (ttype == '左书名号'):
        ...
        book = _light_builtin.列表获取(tokens, (pos + 2))
...
def parse_return_stmt(tokens, pos):
    ...
    if (p < _light_builtin.列表长度(tokens)):
        ...
        node = make_return(_light_builtin.列表获取(esult, 0))
        p = _light_builtin.列表获取(esult, 1)
        return [node, p]
    node = make_return(None)
    return [node, p]
    ...
    if (p < _light_builtin.列表长度(tokens)):
        ...
        if (_light_builtin.字典获取(eq_tok, '种别') == '关键字'):
            if ((_light_builtin.字典获取(eq_tok, '值') == '等于') or (_light_builtin.字典获取(eq_tok, '值') == '为')):
                p += 1
    esult = parse_expr(tokens, p)
    node = make_var_decl(var_name, _light_builtin.列表获取(esult, 0))
    return [node, _light_builtin.列表获取(esult, 1)]
...
def parse_assign(tokens, pos):
    ...
    if (p < _light_builtin.列表长度(tokens)):
        ...
        if ((_light_builtin.字典获取(as_tok, '种别') == '关键字') and (_light_builtin.字典获取(as_tok, '值') == '为')):
            p += 1
    esult = parse_expr(tokens, p)
    node = make_assign(var_name, _light_builtin.列表获取(esult, 0))
    return [node, _light_builtin.列表获取(esult, 1)]
...
def parse_compound_assign(tokens, pos, op):
    ...
    node = make_compound_assign(var_name, op, _light_builtin.列表获取(esult, 0))
    return [node, _light_builtin.列表获取(esult, 1)]
...
def parse_stmt(tokens, pos):
    ...
    node = make_expr_stmt(_light_builtin.列表获取(esult, 0))
    return [node, _light_builtin.列表获取(esult, 1)]
...
def parse(tokens):
    ...
    while (pos < tok_count):
        ...
        stmt = _light_builtin.列表获取(esult, 0)
        if (stmt != None):
            _light_builtin.列表追加(stmts, stmt)
        pos = _light_builtin.列表获取(esult, 1)
...
def get_output(state):
    ...
    while (i < _light_builtin.列表长度(lines)):
        if (i > 0):
            esult = (esult + '\n')
        esult = (esult + _light_builtin.列表获取(lines, i))
        i += 1
    return esult
...
def gen_expr(state, node):
    ...
    if (node_type == 'binary_op'):
        ...
        ight = gen_expr(state, _light_builtin.列表获取(node, 3))
        py_op = op
        if ((op == '加') or (op == '+')):
            py_op = '+'
        elif ((op == '减') or (op == '-')):
