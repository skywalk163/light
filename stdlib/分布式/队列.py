...
为什么在 分布式/ 下自己实现，而不是改 stdlib/并发.light：
  ...
  所以这里在 分布式/ 下实现最小有界异步队列 + 滑动窗口限流。

系统边界登记（外发任务 §4.5：哪几行、为什么是真边界、期望的中文内置名）：
- `asyncio.Queue`（有界队列）：光明无带容量上界的异步队列原语（并发.light 没有，
  F9 又不许改它）；背压语义「满=等」就是 `await put` 挂起生产者。
  期望内置：异步有界队列类（S3 移交 E9，见交付报告 §7.2 欠账条目）。
- `time.monotonic`（滑动窗口过期判定）：窗口滑过的事件按单调时钟剔除。
  期望内置：单调时钟()（已在 任务书/缺失内置清单.json 立项）。

**实现位置如实说明**：当前背压/限流判据实测的是 Python 标准库的语义
（asyncio.Queue.put / list+pop），光明侧只有转发——定级 partial 的原因之一，
见 任务书/分布式判据清单.json 背压/限流 的 备注。
