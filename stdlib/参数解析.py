...
def 添加参数(解析器, 名称, 短名称=None, 值类型=None, 默认值=None, 描述='',
              必需=False, 取值列表=None, 多值=False, 标志=False, 计数=False):
    """添加命令行参数（函数式 API，对齐 .light；值类型 对齐 .light 参数名）"""
    if 值类型 is None:
        值类型 = str
    return 解析器.添加参数(名称, 短名称, _解析值类型(值类型), 默认值, 描述, 必需,
                              取值列表, 多值, 标志, 计数)


def 添加位置参数(解析器, 名称, 值类型=None, 默认值=None, 描述='',
                  可选=False, 多值=False):
    """添加位置参数（函数式 API，对齐 .light）"""
    if 值类型 is None:
        值类型 = str
    return 解析器.添加位置参数(名称, _解析值类型(值类型), 默认值, 描述, 可选, 多值)
...
def _解析值类型(值类型):
    """值类型兼容 type 对象与字符串名（'整数'/'浮点数' 对齐 .light 参数转类型）"""
    if isinstance(值类型, str):
        return _类型名映射.get(值类型, str)
    return 值类型


def 添加子命令(解析器, 名称, 描述=''):
    """添加子命令（函数式 API，对齐 .light）"""
    return 解析器.添加子命令(名称, 描述)


def 解析(解析器, 参数列表=None):
    """解析命令行参数（函数式 API，对齐 .light）"""
    return 解析器.解析(参数列表)


def 获取(解析器, 参数名, 默认值=None):
    """获取参数值（函数式 API，对齐 .light）"""
    return 解析器.获取(参数名, 默认值)


def 打印帮助(解析器):
    """打印帮助（函数式 API，对齐 .light）"""
    return 解析器.打印帮助()


def 帮助文本(解析器):
    """获取帮助文本（函数式 API，对齐 .light）"""
    return 解析器.帮助文本()


def 用法文本(解析器):
    """获取用法文本（函数式 API，对齐 .light）"""
    return 解析器.用法文本()
