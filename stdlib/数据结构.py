...
# -----------------------------------------------------------------------------
# 函数式对齐层（对齐 stdlib/数据结构.light 函数式 API，双路径一致）
# 注：优先队列/单链表/二叉搜索树的函数式包装转发到对应类；
# 栈/队列/双端队列的别名转发到本模块已有的函数式函数。
# -----------------------------------------------------------------------------

def 压入(栈: list, 值: Any) -> list:
    """压入栈顶（.light 对齐别名，同 入栈）"""
    栈.append(值)
    return 栈


def 弹出(栈: list) -> Any:
    """弹出栈顶元素（.light 对齐别名，同 出栈）"""
    return 出栈(栈)


def 顶部(栈: list) -> Any:
    """查看栈顶（.light 对齐别名，同 查看栈顶）"""
    return 查看栈顶(栈)


def 队首(队列: deque) -> Any:
    """查看队首（.light 对齐别名，同 查看队首）"""
    return 查看队首(队列)


def 左入队(双端队列: deque, 值: Any) -> deque:
    """从左端插入（.light 对齐别名，同 左入）"""
    左入(双端队列, 值)
    return 双端队列


def 右入队(双端队列: deque, 值: Any) -> deque:
    """从右端插入（.light 对齐别名，同 右入）"""
    右入(双端队列, 值)
    return 双端队列


def 左出队(双端队列: deque) -> Any:
    """从左端取出（.light 对齐别名，同 左出）"""
    return 左出(双端队列)


def 右出队(双端队列: deque) -> Any:
    """从右端取出（.light 对齐别名，同 右出）"""
    return 右出(双端队列)


def 弹出元素(栈: list) -> Any:
    """弹出栈顶元素（元素语义，对齐 .light 弹出元素）"""
    return 出栈(栈)


def 出队元素(队列: deque) -> Any:
    """从队首取出元素（元素语义，对齐 .light 出队元素）"""
    return 出队(队列)


def 左出队元素(双端队列: deque) -> Any:
    """从左端取出元素（元素语义，对齐 .light 左出队元素）"""
    return 左出(双端队列)


def 右出队元素(双端队列: deque) -> Any:
    """从右端取出元素（元素语义，对齐 .light 右出队元素）"""
    return 右出(双端队列)


def 优先出队元素(pq: 优先队列) -> Any:
    """取出优先级最小的元素（元素语义，对齐 .light 优先出队元素）"""
    return pq.出队()
...
def 优先入队(pq: 优先队列, 元素: Any, 优先级: int = 0) -> 优先队列:
    """加入元素（参数序：元素, 优先级，对齐 .light）"""
    pq.入队(优先级, 元素)
    return pq


def 优先出队(pq: 优先队列) -> Any:
    """取出优先级最小的元素"""
    return pq.出队()


def 优先队列大小(pq: 优先队列) -> int:
    """获取优先队列元素数量"""
    return pq.大小()
...
def 尾部插入(链表: 单链表, 值: Any) -> 单链表:
    """尾部插入"""
    链表.尾部插入(值)
    return 链表


def 头部插入(链表: 单链表, 值: Any) -> 单链表:
    """头部插入"""
    链表.头部插入(值)
    return 链表
...
def 链表获取(链表: 单链表, 位置: int) -> Any:
    """获取指定位置元素"""
    return 链表.获取(位置)


def 链表查找(链表: 单链表, 值: Any) -> int:
    """查找值的索引（未找到返回 -1）"""
    return 链表.查找(值)


def 删除指定值(链表: 单链表, 值: Any) -> 单链表:
    """删除指定值（原地）"""
    链表.删除指定值(值)
    return 链表
...
def 二叉插入(树: 二叉搜索树, 值: Any) -> 二叉搜索树:
    """插入值"""
    树.插入(值)
    return 树


def 二叉查找(树: 二叉搜索树, 值: Any) -> bool:
    """查找值"""
    return 树.查找(值)


def 二叉中序遍历(树: 二叉搜索树) -> List[Any]:
    """中序遍历"""
    return 树.中序遍历()


def 二叉删除(树: 二叉搜索树, 值: Any) -> 二叉搜索树:
    """删除值（原地）"""
    树.删除(值)
    return 树
...
__all__ = [
    ...
    # 函数式对齐层（对齐 .light）
    '压入', '弹出', '弹出元素', '顶部', '队首',
    '出队元素', '左出队元素', '右出队元素', '优先出队元素',
    '左入队', '右入队', '左出队', '右出队',
    '创建优先队列', '优先入队', '优先出队', '优先队列大小',
    '创建链表', '尾部插入', '头部插入',
    '链表大小', '链表获取', '链表查找', '删除指定值',
    '创建二叉搜索树', '二叉插入', '二叉查找', '二叉中序遍历', '二叉删除',
]