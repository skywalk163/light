...
> **已修复（T7A / 2026-09-06）**：`_gen_typed_binary_op` INT 快路径对 `/`/`除` 改为
> fdiv + `_create_float_dv_fast`（type=2），通用路径对两侧调 `dv_to_float` 再
> `dv_div`，确保 int/int 除法返回 FLOAT DV；下游 float 算术类型标记一致。
> stdlib workaround 已移除（自包含内联改回跨段调用）。反跑 `git stash` 立红
> 确认。定向测试 `test_codegen_float_slot_O0.py` 全绿。
...
- **Workaround**（已移除）：将所有跨段 float 依赖改为**自包含内联计算**——每个函数独立
...
- **归属**：codegen 类型推断/slot 管理，归 T7 修复。**已修复（T7A / 2026-09-06）**

#### T5A-02：int/int 除法结果在后续算术中被零初始化

> **已修复（T7A / 2026-09-06）**：`_gen_typed_binary_op` INT 快路径对 `/`/`除`
> 改为 fdiv + `_create_float_dv_fast`（type=2），与 `_infer_expr_type` 返回
> FLOAT 一致；通用路径对两侧调 `dv_to_float` 再 `dv_div`。stdlib workaround
> 已移除（`(x+0.0)/n` 改回 `x/n`）。反跑 `git stash` 立红确认。
...
- **Workaround**（已移除）：统一使用 `(总计 + 0.0) / n` 模式——先将分子通过 `+ 0.0` 转为
  float dv，再做除法，确保结果为 float 类型标记。
- **归属**：codegen 除法类型推断，归 T7 修复。**已修复（T7A / 2026-09-06）**

#### T5A-03：`_light_builtin.平方根()` 返回值存入变量后在算术中被零初始化

> **已修复（T7A / 2026-09-06）**：被 T5A-02 修复覆盖——int/int 除法类型标记
> 修复后，float DV 的 slot 管理一致，builtin 返回值存入变量后算术不再零初始化。
> stdlib workaround 已移除（内联牛顿法改回 `_light_builtin.平方根()` 调用）。
...
- **Workaround**（已移除）：在 `相关系数` 和 `标准化` 中用**内联牛顿法 sqrt** 替代
  `_light_builtin.平方根()` 调用：20 次迭代 `(r + val / r) / 2.0`，纯局部 float
  算术无函数调用。
- **归属**：codegen builtin 返回值 slot 管理，归 T7 修复。**已修复（T7A / 2026-09-06）**

#### T5A-04：float 字面量作为用户段函数参数时被零初始化

> **已修复（T7A / 2026-09-06）**：被 T5A-02 修复覆盖——int/int 除法类型标记
> 修复后 float DV slot 管理一致，float 参数传递不再零初始化。stdlib
> workaround 已移除（int 参数改回 float 参数）。移除 workaround 后完整
> stdlib 对拍全绿确认。
...
- **Workaround**（已移除）：`百分位数` 段改为接收 int 参数，内部 `设 p 为 百分位 + 0.0`
  转 float。`四分位数` 调用改为传 `25`/`50`/`75`（int）。
- **归属**：codegen 参数传递类型推断，归 T7 修复。**已修复（T7A / 2026-09-06）**
...
> **已修复（T7A / 2026-09-06）**：被 T5A-02 修复覆盖——int/int 除法类型标记
> 修复后 float DV slot 管理一致，float 参数不再零初始化。stdlib workaround
> 已移除（安全防护 `如果 值 <= 0.0` 删除，内联 Taylor 展开改为
> `_light_builtin.自然对数()` / `自然对数(x)/自然对数(底数)` 跨段调用）。

- **现象**：`自然对数(2.718...)` 超时——参数被零初始化后 `当 值 > 2.0` 条件
  恒为真（0 不大于 2.0 但泰勒展开对 0 无限循环），导致死循环。
- **根因**：与 T5A-04 同类——float 字面量作为函数参数被零初始化。
- **Workaround**（已移除）：添加安全防护 `如果 值 <= 0.0: 返回 0.0`，避免死循环。
  实际调用时通过 int 参数或 `+ 0.0` 转换绕过。
- **归属**：同 T5A-04，归 T7 修复。**已修复（T7A / 2026-09-06）**
...
- **状态**：**已修复（T7B / 2026-09-06）**。codegen 新增「参数写回」链：
  被调侧在返回出口把**未被重新绑定**的形参槽写回 `%args[i]`，调用侧再把
  `%args[i]` 搬回实参变量（`_emit_param_writeback` + `_emit_args_writeback`），
  O0 下列表/字典 REF 传递语义与 Python 一致。反跑 `test_codegen_ref_dict_O0.py
  ::test_T5C02_REF参数追加写回_O0` 绿。
...
- **状态**：**已修复（T7B / 2026-09-06）**。codegen 索引赋值走新 runtime
  分派 `dv_index_set`（列表 → `dv_list_set`、字典 → `dv_dict_set`），不再把
  列表壳整体改写成 dict。workaround 已回退：`随机.light`（随机洗牌/抽样）、
  `集合操作.light`（打乱）的 `设置()` 全部改回 `设 列表[i] 为 值`，固定种子
  位级对拍保持。反跑 `test_T5C03_REF索引赋值_O0` 绿。
...
- **状态**：**已修复（T7B / 2026-09-06）**。字典新增键经函数参数的写回由
  参数写回链覆盖（同 T5C-02）。workaround 已回退：`对象池缓存.light` 缓存
  家族改回 `{键: 值}` / `{键: [值, 截止]}` 纯字典存储；新注册 `字典删除`
  builtin（`dv_dict_remove`，codegen 声明+映射）支撑删除/清空。
  反跑 `test_T5C04_字典新增键REF写回_O0` 与 `test_对象池缓存_冒烟_O0` 绿。
...
- **归属**：codegen 变参，归 T7。
- **状态**：**已修复（T7B / 2026-09-06）**。根因：parser 把 `接收 *x` 产成
  `Parameter(name='*x')`，codegen 未剥星号 → 局部变量名带星号 → 引用落入
  「未知标识符当字符串」兜底。修复：codegen 剥星号并把第 i 个起的剩余实参经
  runtime `dv_collect_varargs`/`dv_collect_kwargs` 收集为列表/字典绑定。
  workaround 已回退：`集合.多个并集/多个交集`、`集合操作.合并`、
  `集合工具.链字典` 改回变参（对齐 .py）。反跑 `test_T5C05_变参绑定_O0` 绿。
...
- **现象（同一根因的两种表现）**：
  1. **读脏值**（仅 POSIX 暴露）：`集合.移除元素` 对**存在**的元素也抛 `值错误`。
  2. **写丢失**（**双平台**均存在）：`集合操作.分组` 每个分组恒少收 1 个元素 ——
     `分组(列表(1, 1, 2))` 得 `{1:[1], 2:[]}`，而应为 `{1:[1,1], 2:[2]}`。
- **根因**：`字典设置(字典, 键, 值)` 会重绑/释放底层缓冲，使「**写回前**取到的
  列表句柄」失效。事后对该句柄的**读**是未定义行为，**写**则落进已释放缓冲。
  - `移除元素` 原实现在 `字典设置` **之后**才比较 `长(列表)` 与新长度：
    Windows 上该块内存读作 `0`（`0 != 2`，不抛错 —— **侥幸通过**）；
    POSIX/glibc 上被复用后读作 `2`（与新列表同长）→ 误判「没删掉」而抛错。
  - `分组` 原实现在 `字典设置` **之后**才 `追加(组, …)`，该次写入丢失。
- **取证**：探针打印「写回后旧句柄长度」——Windows `0` / Linux `2`。同一份 UB
  读出两种脏值，是「Windows 绿、POSIX 红」的**直接证据**（探针见交付记录）。
- **修复**（`.light` 层 workaround，不碰 codegen，归 T7）：
  - `移除元素`：写回**前**先 `设 原长 为 长(列表)` 快照，比较前置。顺带使
    「元素不存在」时不再产生半截写入，与 Python `set.remove` 抛 KeyError 且
    不改集合的语义一致。
  - `分组`：新建组改为**先追加、后写回**；已存在的组走「字典读出的句柄 +
    `追加`」（T5C-02 认定该路径生效）。
- **回归测试**：`test_集合_移除元素_句柄失效回归_O0`（含「缺失元素仍须抛错」的
  `尝试/捕获` 断言，防止修复把语义改成静默成功）、
  `test_集合操作_分组_组内元素不丢失_O0`。
- **注意（测试盲区）**：原 `test_集合操作_排序_打乱_去重_对拍_O0` 只校验 `分组`
  的**键数**（恒为 2），掩盖了组内元素丢失；新用例补齐**值**维度。
- **归属**：根因仍在 codegen 的字典写回 / 句柄失效语义，归 T7；本批以 `.light`
  层 workaround 修，与 T5C-02~04 同一处置口径。
- **状态**：**部分已修复（T7B / 2026-09-06），拆两个维度**：
  - **REF 函数参数维度**（跨函数把 dict/list 传入、函数内 `字典设置` 改到
    实参上）：由参数写回链根因修复，与 T5C-02/03/04 同批生效。
  - **函数内句柄失效维度**（同一函数内先读句柄、后 `字典设置` 替换值使旧句柄
    悬空）：T7B 未消除（`dv_dict_set` 替换值会释放旧值，先读出的浅拷贝句柄
    仍指向已释放缓冲）。该维度按任务许可**保留 .light 层防御**（`移除元素`
    前置快照 + `分组` 先追加后写回），并新增 `字典删除` builtin 后缓存家族
    字典化绕开旧句柄复用；对外行为 Windows/POSIX 双平台一致
    （`test_T5C07_字典设置句柄失效_公共API_O0`、`test_T5C07_分组_组内元素
    不丢失_O0` 绿）。**全量根因消除需 runtime 字典值所有权/引用计数重构，
    不在 T7B 范围**。
...
- 函数值/回调不可用（编译期「未定义的段落」）→ 对象池缓存的装饰器类返回
  `空`/池本身，仅标注能力边界。
- **POSIX 实测账（T5C-POSIX 专项，2026-09-06 收口）**：
  - **首轮（随 T6B）**：`dv_random` 系列（MT19937 纯整数运算 + `dv_timestamp`
    播种）经 `test_随机_固定种子42逐位对拍_O0` **passed**；同轮全量原生腿回归
    中 `test_集合_运算对拍_O0`（`移除元素`）暴露原生腿 `值错误`。已确认
    **非 T6B 引入**（T6B 的 codegen/runtime 改动仅涉 time/sys builtin 与
    `sys.stderr` 链改写，git diff 已核零集合/移除相关代码），根因是 T5C 集合族
    缺陷，登记为 **T5C-07**。
  - **修复后（本轮）**：`移除元素` 修好，并顺带修掉同根因的 `分组` 组内元素
    丢失（该缺陷**双平台**均存在，此前未被测试覆盖）。POSIX 全量原生腿回归
    **36 passed / 0 failed**（T5A + T5B + T5C + T6B）；Windows 同组合亦
    **36 passed**。T5C「未在 POSIX 实测」的缺口由此关闭。
  - **环境**：Ubuntu 24.04.4 / clang 18.1.3 / 3.9GB 空闲 / 4 核；
    本机 Windows 对照为 clang 22.1.8 + MSVC 14.29 + Win10 SDK 10.0.19041
    （T5A/T5B/T5C/T6B 两套工具链版本不同，是本次能暴露平台相关 UB 的前提）。


## R10-12d T6A：re.light 正则引擎 + 正则表达式/格式化/模板 .light 化 — O0 缺陷与能力边界（2026-09-06）

### 概述

T6A 新增 `stdlib/re.light`（纯光明回溯型正则引擎，约 1150 行：字符类/
区间/量词 `{n},{n,},{n,m}`/贪婪与懒惰/分支/分组捕获与反向引用/多行锚点/
忽略大小写），并把 `stdlib/正则表达式.light`、`stdlib/格式化.light`、
`stdlib/模板.light` 三个「decl 0 空壳」补为真实现。编译运行与测试全部在
POSIX 实机 `192.168.0.86`（clang 18.1.3）完成，T6A 是第一个全程 Linux
实机验证的原生腿批次。测试 `tests/unit/test_T6a_正则文本模块_原生腿.py`
14 例全绿（含 3 例空壳判红）。

### 缺陷清单（本轮新发现，均以 stdlib 层 workaround 绕过，归 T7）

#### T6A-01：负数存入列表读出损坏（-1 → 1）

- **现象**：`返回 [-1]`、`盒.追加(-1)` 两条路径存入后 `盒[0]` 读出 `1`
  （mini8 实测）；正数（5/7）正常。`模板.light` 的 `_找子` 用 `-1` 当
  「未找到」哨兵 → 调用方判断 `开 < 0` 永假 → 渲染循环死循环。
- **Workaround**：`_找子` 改用「串长」作未找到哨兵（非负越界值）；
  re.light 组表 `追加(-1)` 只写不读判断，不受影响。
- **归属**：runtime 负数 DV 装箱/codegen 列表元素写路径，归 T7。
- **状态**：**已修复（T7B / 2026-09-06）**。根因是 **codegen 一元负号被静默
  忽略**：`-1` 的 AST `UnaryOp`（operator `-`）在 `_gen_typed_unary_op` 中
  无分支 → 按正数原样下发（非 runtime DV 装箱问题，runtime 零改动）。
  修复：一元负号对数值操作数发射 `dv_sub(0, x)`。workaround 已回退：
  `模板.light` 的 `_找子` 未找到哨兵改回 `-1`（3 处调用判断同步从 `>= n`
  改为 `< 0`）。反跑 `test_T6A01_负数字面量_存储读取_O0` 与模板渲染用例绿。

#### T6A-02：当循环 + 嵌套分支内的标量赋值，循环退出后读取损坏（4 → 1）

> **已由 T9A/T7A/T7B 间接修复（T7C / 2026-09-06 验证确认）**：T9A 段名模块隔离
> 修复后，codegen 使用 alloca 栈槽存储局部变量（非 SSA 寄存器），不存在 PHI
> 节点——原「PHI 节点管理错误」根因分析有误。mini17 在当前代码上 O0 编译运行
> 输出 `2`（正确，非 `1`）。workaround 已移除：`格式化.light` 的 `文本去除缩进`
> 改回标量变量 `最小`（移除 `最小盒` 单元素列表 workaround）。`test_codegen_controlflow_O0.py`
> 2 个用例全绿，`test_T6a_正则文本模块_原生腿.py` 17/17 全绿。

- **现象**：`当` 循环体内「如果/否则」两层嵌套中的 `设 最小 为 cnt`
  （cnt=4），循环退出后读 `最小` 得 `1`（mini17 最小复现）；循环体**顶层**
  的标量赋值正常（mini16 全对）。布尔化特征：非零 → 1。
- **Workaround**（已移除）：`格式化.light` 的 `文本去除缩进` 把「最小公共缩进」改放
  **单元素列表**（`最小盒`），min 比较改为分支内追加非负值。
- **归属**：codegen 循环标量存活区间，归 T7。**已由 T9A/T7A/T7B 间接修复**

#### T6A-03：模块级 `设` 初始化的字符串常量不可靠

> **已修复（T7C / 2026-09-06）**：根因是 **模块初始化顺序错误**——`compiler.py`
> 的 `compile_modules_typed` 中 `all_module_list` 按 `modules.values()` 原始顺序
> 收集，主模块可能排在被导入模块之前，导致 `__light_init` 中主模块的顶层语句
> （如读取导入模块的全局变量、调用导入段）先于被导入模块的全局初始化执行。
> 修复：将主模块移到 `all_module_list` 末尾，使被导入模块的全局初始化先执行。
> 反跑：`git stash` 回退修复后，模块级常量读取立即返回 `空`。workaround
> （段落开头局部构造 槽开/槽闭/单开/单闭）仍保留在 `模板.light` 中——因协作
> 红线限制不动 `模板.light`（归下一批），`格式化.light` 的 workaround 不涉及
> 此缺陷。

- **现象**：`模板.light` 模块级 `设 槽开 为 字符自码位(123) 加上 ...`
  在段落里读到垃圾值（桩测 `_找子` 在下标 3 命中，实为按「无」匹配），
  渲染死循环。
- **Workaround**（模板.light 仍保留，归下一批移除）：删除模块级常量，改在段落开头局部构造
  （`渲染模板`/`模板变量替换` 各自构造 槽开/槽闭/单开/单闭）。
  re.light 全模块即此风格（无模块级设语句）。
- **归属**：codegen 模块级初始化，归 T7。**已修复（T7C / 2026-09-06）**
...
> **已修复（T9B / 2026-09-06）**：re.light VM `支` 处理器增加组表/回点快照恢复——
> 分支1递归调用可能通过 `保存` 指令修改 `组表`、通过嵌套 `循支` 修改 `回点`，
> 分支1失败后恢复快照再试分支2。IPv4 模式对 `999.1.1.1` 正确判定不匹配。
> `test_relight_engine_O0.py::test_t6a07_mini20_ipv4_999` 全绿。

- **现象**：IPv4 模式
  `^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}...$` 对
  `999.1.1.1` 误判通过（mini20）；去掉第三分支 `[01]?...` 后（mini19）
  判定正确。简化分支的 `{3}` 重复正常。
- **根因**：re.light VM `支` 处理器中，分支1递归调用修改 `组表`（`保存` 指令）
  和 `回点`（嵌套 `循支`）后，分支1失败未恢复，分支2看到污染状态。
- **修复**：T9B `支` 处理器尝试分支1前快照 `组表` 和 `回点`，分支1失败后恢复再试分支2。
- **Workaround（已移除）**：`验证IP地址` 保留 re 模式版；测试反例改用 `192.168.1`。
  IP 反例全集（999/256/3段/0.0.0.0）已恢复测试。
- **归属**：re.light VM 回溯状态管理。**已修复（T9B / 2026-09-06）**

#### T6A-08：builtin `整除` 结果参与后续算术被零初始化（T5A 同族）

> **已修复（T7A / 2026-09-06）**：被 T5A-02 修复覆盖——int/int 除法类型标记
> 修复后 builtin `整除` 结果参与后续算术不再零初始化。stdlib workaround 已移除
> （减二循环改回 `整除()` 调用）。`test_T6a_正则文本模块_原生腿.py` 17/17 全绿。

- **现象**：`文本居中` 的 `整除(宽度 - n, 2)` 恒 0 → 左填充丢失。
- **Workaround**（已移除）：减二循环求整除（`文本居中` 内联 3 行）。
- **归属**：同 T5A int/int 除法缺陷，归 T7。**已修复（T7A / 2026-09-06）**
...
> **已由 T9A 间接修复（T7C / 2026-09-06 验证确认）**：T9A 段名模块隔离修复后，
> 构造 60+ 次跨模块调用 smoke 程序（`文本居中`/`文本左对齐`/`文本右对齐` 交替调用），
> 连续 20 次复跑全部 PASS，未再出现返回值污染。根因确认为 T6A-09（段名符号编号
> 冲突）的同族表现——全局编号冲突导致不同模块的同名段映射到同一 LLVM 函数，
> 调用结果互相覆盖。段名隔离后此问题消除。

- **现象**：同一 .light 程序内，`文本居中` 等函数在完整 smoke（60+ 调用）
  中偶发返回缺左填充的串；相同 code 在独立小程序中稳定正确；pytest
  同 code 同 helper 复跑结果可随调用顺序翻转（本轮以「调换断言顺序」
  通过）。**未最小化定界**，疑似与 T6A-09 同根（全局槽/寄存器复用）。
- **缓解**（已解除）：测试遵循 T5A 范式「每用例独立编译运行」，用例保持小型；
  大程序场景（smoke main2.light）留档于任务纪要。
- **归属**：codegen 全局槽分配/调用约定，归 T7。**已由 T9A 间接修复**

#### T6A-11：re.light 回溯VM 列表操作内存安全缺陷（Windows 堆损坏 0xC0000374）

> **已修复（T9B / 2026-09-06）**：re.light VM `循支` 处理器增加组表/回点快照恢复——
> body 失败后恢复快照（保留 `[pc,pos]` 防出口跳回死循环，清除 body 的 stale 条目）再试出口。
> 新增 `re_内部_列复制` 辅助段（列表浅复制）用于快照。修复后连续 20 次正则验证调用
> Windows 稳定运行不崩，POSIX valgrind **Invalid read/write = 0**（原 11681 含 Invalid
> read/write）。`test_relight_engine_O0.py::test_t6a11_20次连续验证不崩` 全绿。
> 拆分的 4 个验证函数测试已合并回 1 个完整 20 次调用测试。
...
- **根因**：re.light 回溯 VM `循支` 处理器中，body 失败后 body 内嵌套 `循支`
  追加的 stale 回点条目未清理，影响出口路径；列表可变状态在回溯时未恢复。
  re.light 注释自称"列表不可变语义天然支持回溯撤销"，但实际列表是引用传递+原地修改
  （`追加`/`设置`），设计假设不成立。
- **修复**：T9B ①新增 `re_内部_列复制` 辅助段（列表浅复制）；②`循支` 处理器追加
  `[pc,pos]` 后快照 `组表` 和 `回点`（含刚追加的条目），body 失败后恢复（保留
  `[pc,pos]` 防出口跳回死循环，清除 body 的 stale 条目）再试出口；③`支` 处理器
  同步增加快照恢复（T6A-07 同根因）。
- **valgrind 验证**（Ubuntu 192.168.0.86，valgrind 3.22.0，clang 18.1.3）：
  修复版连续 20 次调用 **Invalid read/write = 0**，ERROR SUMMARY 11035 全部为泄漏
  上下文（无内存安全错误）。仍存在 definitely lost ~747KB / indirectly lost ~9.5MB，
  泄漏路径 `dv_clone`←`dv_list_get` 属 runtime 引用计数与列表元素访问 clone 机制
  交互问题（归 T7D），非 re.light 引擎缺陷。
- **Workaround（已移除）**：测试 `test_正则_验证函数` 拆为 4 个小测试（每用例 ≤6 次
  调用）→ 已合并回 1 个完整 20 次调用测试 `test_正则_验证函数_完整20次`。
  IP 反例测试（`192.168.1` 等）已恢复。
- **归属**：re.light 回溯 VM 列表生命周期管理。**已修复（T9B / 2026-09-06）**
  （内存安全根因消除；残余泄漏归 T7D runtime）

### 能力边界（诚实声明）

- re.light 支持字符类/区间/否定类/`.`/锚点/量词（含懒惰）/分支/分组
  （含 `(?:)` 非捕获）/反向引用 `\1`-`\9`/忽略大小写/多行/点号全行；
  **不支持**：前后断言（lookaround）、命名分组 `(?P<name>)`、
  `\b` 词边界、Unicode 属性类。步数预算 200000（防灾难回溯）。
- `模板` 类以「模板串即对象」形态提供（原生腿无类方法分派）；
  `{{ 名字 }}` 槽名两侧空白统一去除（.py 仅默认值分支去空白）。
- `模板循环` 对纯列表项仅替换 `{{item}}`/`${item}`，`{{item.键}}`
  原样保留 —— 与 .py 一致（.py 亦不展开）。
- `验证密码强度` 为纯启发式（长度/大小写/数字/符号计分），与 .py
  分档口径对齐但不做字典检查。
- `文本换行` 不做 textwrap 长词内断词微调（与 .py 的 suspend 语义
  差异已标注）；expand_tabs/replace_whitespace 选项不承诺。
...
## R10-12e T7B：REF 参数写回 / 字典句柄 / 变参 / 负数字面量根因修复（2026-09-06）

### 概述

T5C/T6A 过程中登记在案、归 T7 的 6 个 codegen 缺陷，在 T7B 一并根因修复。
改动集中在 `src/llvm/codegen_typed.py`（参数写回链 + 索引赋值分派 + 变参绑定
+ 一元负号 + `字典删除` 注册）与 `src/llvm/runtime_typed.c`（`dv_index_set`/
`dv_collect_varargs`/`dv_collect_kwargs` 三个独立新函数，**未碰 TLS 段**）。
对应 stdlib workaround 回退（见各缺陷条目状态）。逐条状态见本文件 T5C-02~05、
T5C-07、T6A-01。

### 变更清单

1. **参数写回链**（T5C-02/04 根因）：段落函数返回出口 `_emit_param_writeback`
   把**未被重新绑定**的形参槽写回 `%args[i]`（带 `i < num_args` 越界保护，缺参
   默认值场景不越界写）；调用侧 `_emit_args_writeback` 在 `stackrestore` 前把
   `%args[i]` 搬回实参变量（仅「实参是局部变量标识符」的位置）。`_collect_
   rebound_names` 是排除表的关键——T7B 实测发现它只查 `target`/`variable` 字段，
   漏了主力赋值形态 `VariableDeclaration`（重绑目标在 **str 字段 `name`**），
   导致 rebind 恒为空集、**全部形参被登记写回**，深度回溯递归（re.light 回溯
   VM `跑`）的中途态被写回污染调用方（`a*a`/`[a-z]+x`/提取邮箱 全 0 匹配的
   元凶）。补齐 `name`/`destructure_names`/`Assignment.target`/
   `ForeachStatement.variable` 且不下钻嵌套段落/类。
2. **索引赋值分派**（T5C-03）：`设 容器[i] 为 值` 走新 runtime `dv_index_set`，
   按 obj 真实类型分派（list→`dv_list_set`、dict→`dv_dict_set`）；此前无条件
   `dv_dict_set` 会把列表壳整体改写成 dict（元素丢/读空）。
3. **变参绑定**（T5C-05）：parser 把 `接收 *x` 产成 `Parameter(name='*x')`；
   codegen 剥星号并把第 i 个起剩余实参经 `dv_collect_varargs`（列表）/
   `dv_collect_kwargs`（字典）收集。
4. **一元负号**（T6A-01）：`_gen_typed_unary_op` 对 `-` 发射 `dv_sub(0, x)`
   （此前被静默忽略 → `-1` 按 `1` 存）。runtime 零改动。
5. **`字典删除` builtin 注册**：runtime `dv_dict_remove` 早已存在但未注册；
   补声明 + builtin 映射，供缓存家族字典化删除用。

### 移除的 workaround 清单

| 模块 | 原 workaround（T5C-02~05 / T6A-01） | 回退为 |
|---|---|---|
| 集合.light | `多个并集/多个交集` 固定双参（T5C-05） | `接收 *集合们` 变参（对齐 .py） |
| 集合操作.light | `合并` 固定双参（T5C-05）；`打乱` 交换用 `设置()`（T5C-03） | `*列表们` 变参；`设 结果[i] 为 …` 索引赋值 |
| 集合工具.light | `链字典` 固定双参（T5C-05） | `*映射们` 变参（对齐 .py） |
| 随机.light | `随机洗牌/随机抽样` 用 `设置()`（T5C-03） | 索引赋值（固定种子位级对拍保持） |
| 对象池缓存.light | 缓存家族「键/值/截止 并行列表」（T5C-04） | `{键: 值}` / `{键: [值, 截止]}` 字典存储 + `字典删除` |
| 模板.light | `_找子` 未找到哨兵用「串长」（T6A-01） | 改回 `-1`（3 处调用同步 `>= n` → `< 0`） |
| 集合.light 移除元素 / 集合操作.light 分组 | 前置快照 / 先追加后写回（T5C-07，**保留**） | 保留为防御（见 T5C-07 状态：函数内句柄失效维度未在 T7B 消除） |

### 验证（O0 真编译真跑，双平台）

- 新增 `tests/unit/test_codegen_ref_dict_O0.py` 8 用例（6 缺陷最小复现 + 分组值
  维度 + 回溯递归写回回归），Windows 与 Ubuntu 均全绿。
- Windows（clang 22.1.8 + MSVC 14.29 + SDK 10.0.19041）：T5A/T5B/T5C/T6A/T6B
  全原生腿 **53 passed** + 新增 8 = 61。
- POSIX `192.168.0.86`（Ubuntu 24.04.4 / clang 18.1.3）：同组合 **61 passed /
  0 failed**（含全部 workaround 回退后的集合族/模板/对象池缓存）。
- codegen/runtime 之外的回归面：目标模块均非「纯光明」，python 腿走同名 `.py`，
  不受影响。

### 仍存在的缺口

- **T5C-07 函数内句柄失效维度**：`dv_dict_set` 替换值释放旧值使先读出的浅拷贝
...
## R10-12g R11A：通用工具族 7 模块真 .light 化 — O0 codegen 缺陷与能力边界（2026-09-06）

**任务编号**：native-utils-R11A ｜ **分支**：`task-R11A`
**范围**：字符串工具 / 数据结构 / 断言工具 / 缓存 / uuid工具 / 进度条 / 数据验证
7 个 `stdlib/*.py` → 同 `.light` 真实现（非 decl-0 空壳），O0 真编译真跑，
与 Python 转译腿逐函数对拍。

### 一、本轮新增 codegen 缺陷（未修，均已在 .light 侧 workaround）

| 编号 | 缺陷 | 最小复现 | 规避 |
|---|---|---|---|
| R11A-01 | **`非` 作用于函数调用失效**：`非 是数字字符("a")` 返回假（应为真）；对字面量/括号表达式正常 | `输出(非 是数字字符("a"))` | 改 `是数字字符(x) == 假`。**✅ 已修复（R12A / 2026-09-06）**：`非` 走 dv_to_bool 真值取反，workaround 已移除 |
| R11A-02 | **内建函数名作局部变量 → 算术静默返 `[]`**：`删除`/`替换`/`插入`/`最小` 等（见 `src/keywords.py` 注册表）用作变量名后 `变量 加上 1` 得 `[]`，整条计算链报废 | `设 删除 为 1` → `删除 加上 1` | 变量名避开内建表，用 `删代价`/`替代价` 等复合名 |
| R11A-03 | **读列表下标 0 赋变量返 `[]`**：`设 x 为 dp[0]` → `[]`（dp[1]/dp[8] 正常） | `设 d 为 [1,2]` → `设 x 为 d[0]` | 多维/DP 用 1-based 偏移 `(r+1)*宽+(c+1)` |
| R11A-04 | **`除以` 对 dict/list 读取值返 `空`**：真除路径对容器取出的值失效，`整除` 正常 | `设 v 为 字典获取(d,"k")` → `v 除以 2` | 改用 `整除` |
| R11A-05 | **跨模块「导入即再导出」codegen 失败**：`从 正则 导入 验证邮箱` 的调用点报「未定义的段落」；同模块内再导出可用 | 跨模块原样再导出 | 薄包装 + 限定调用 `段落 验证邮箱 接收 文本: 返回 正则表达式.验证邮箱(文本)`。**✅ 已修复（R12C / 2026-09-07）**：codegen 记录每模块导入映射（_module_imports）并沿再导出链解析到真正定义，调用点直接调定义端 safe name |
| R11A-06 | **默认参数禁负数字面量**：`接收 甲 = -1` 报「意外的标记」（表达式内负值可用） | `段落 f 接收 甲 = -1:` | 用 `空` 哨兵（`0 == 空` 为假，安全）。**✅ 已修复（R12C / 2026-09-07）**：parser _attach_param_default 支持 '-' + 数字 的负数字面量默认值 |
| R11A-07 | **`且`/`或` 内联条件解析失败且不短路** | `如果 a 且 b:` 复杂内联 | 拆嵌套 `如果` |
| R11A-08 | **字符串内置方法返空**：`.去除空白()`/`.lstrip()`/`.rstrip()`/`.每个单词首字母大写()` 返 `[]`；`.分割()` 无参不做空白切分 | — | 手写循环。**✅ 已修复（R12C / 2026-09-07）**：去除空白→dv_trim 别名、lstrip/title/无参分割→按需发射的 IR 辅助函数（_r12c_lstrip/_r12c_title/_r12c_split_ws，libc isspace/isalpha/tolower 等） |
| R11A-09 | **`转文本(列表)` 在 O0 打印为 `[]` 假象**：元素实际存在（列表句柄序列化缺陷） | `输出(转文本(列表))` | 对拍用 `长(列表)` + 下标逐元素打印。**✅ 已修复（R12C / 2026-09-07）**：转文本 运行时按 DV 类型分派——列表（type 4）走 dv_list_join 加方括号；嵌套容器内层仍走旧路径归 T7 |

| R11A-05 | **跨模块「导入即再导出」codegen 失败**：`从 正则 导入 验证邮箱` 的调用点报「未定义的段落」；同模块内再导出可用 | 跨模块原样再导出 | 薄包装 + 限定调用 `段落 验证邮箱 接收 文本: 返回 正则表达式.验证邮箱(文本)` |
| R11A-06 | **默认参数禁负数字面量**：`接收 甲 = -1` 报「意外的标记」（表达式内负值可用） | `段落 f 接收 甲 = -1:` | 用 `空` 哨兵（`0 == 空` 为假，安全） |
| R11A-07 | **`且`/`或` 内联条件解析失败且不短路** | `如果 a 且 b:` 复杂内联 | 拆嵌套 `如果`。**✅ 已修复（R12A / 2026-09-06）**：根因实为 parser `_parse_logical_expr` 从未接入表达式链（且/或 BinaryOp 节点缺失，右侧被静默丢弃）——已接线（或 < 且 < 比较 三层优先级），并在 codegen 生成短路分支（假 且 报错() 不再求值右侧） |
| R11A-08 | **字符串内置方法返空**：`.去除空白()`/`.lstrip()`/`.rstrip()`/`.每个单词首字母大写()` 返 `[]`；`.分割()` 无参不做空白切分 | — | 手写循环 |
| R11A-09 | **`转文本(列表)` 在 O0 打印为 `[]` 假象**：元素实际存在（列表句柄序列化缺陷） | `输出(转文本(列表))` | 对拍用 `长(列表)` + 下标逐元素打印 |
| R11A-10 | **嵌套容器不持久化**：list-in-list / dict-in-dict 的元素写入不落盘 | `设 d 为 字典获取(外,"k")` → 写 `d` | 扁平化 + 显式返回新容器 |
| R11A-11 | **dict 跨函数写回不可靠**（T5C-07 函数内维度的同类）：函数内改 dict 不回传 | — | 显式 `返回` 新 dict，调用方 `设 c 为 缓存设置(c,k,v)` |

> R11A-02/03 是**静默错值**类（不报错、结果错），危害最高：字符串工具的
> Levenshtein DP 曾因 R11A-02 全部作废（相似度恒 1）、因 R11A-03 首行全丢。

### 二、能力边界（降级实现，已在模块头部注释标注）

| 模块 | 降级项 | 原因 |
|---|---|---|
| uuid工具 | **生成UUID3 / 生成UUID5** 降级为「确定性布局 + 正确版本位」（不逐字符匹配 CPython） | 无按位算子；SHA1 按 UTF-8 文本哈希而非原始字节，无法复刻 `uuid.uuid3` 字节级语义 |
| uuid工具 | **生成UUID1** 降级为「v1 格式的随机 UUID」 | 无 MAC 地址 / 60bit 时间戳源 |
| 断言工具 | **断言抛出异常 / 断言满足条件 / 断言所有满足条件 / 断言任一满足条件 / 断言可调用 / 链式断言** 等 Callable 类 API 降级为抛出明确异常的 stub | 原生腿函数值不可调用 |
| 缓存 | **定时缓存**（刷新回调）、**缓存装饰器** 降级 stub | 函数值不可调用 |
| 进度条 | **实时原地刷新**（`\r` 覆盖）、**显示旋转指示器**（time.sleep + 停止回调）、**迭代进度条**（yield）降级 | 原生 `打印` 追加换行不做原地覆盖；无 sleep 控制；无生成器 |
| 数据结构 | 优先队列 / BST 等「弹出」语义统一为**返回新容器**（元素用 `顶部`/`队首`/`查看左端`/`查看右端` 取） | 容器为值语义 + 显式返回（见 R11A-11） |

### 三、反跑判据

| 判据 | 结果 |
|---|---|
| 7 模块真实现对拍 Python oracle | ✅ `tests/unit/test_原生腿_R11A_通用工具.py` **7/7 绿** |
| 改回 decl-0 空壳（保留签名、函数体 `返回 空`）立即立红 | ✅ **7/7 红**（随后已从备份还原，复跑 7/7 绿） |
| O0 真编译真跑（非 O2） | ✅ 全部走 `compile_light_typed(..., optimize_level=0)` + 真 exe 子进程 |

### 四、遗留缺口

1. R11A-01~11 均为 **workaround 未根因修复**（任务书红线：本轮不动
   `src/llvm/*.py` / `runtime_typed.c`），应移交 codegen 根因批次。
2. uuid v3/v5、断言 Callable 族、进度条实时刷新的真语义需语言层先补
   **按位算子 / 函数值可调用 / yield / 终端控制**。
3. 对拍覆盖为每模块 3-5 个代表性用例，非穷举全函数；字符串工具 90 段、
   数据结构 76 段中长尾函数未逐一单测。
   **✅ 已由 R13C 扩展（2026-09-07）**：字符串工具 +20、数据结构 +15 用例，
   覆盖率约 61%/63%，并登记 R13C-01~04 新发现（见 R13C 章节）。

## R11B：中文工具族真 .light 化 —— 新发现缺陷登记（2026-09-06）

> 批次：R11 S2 原生腿中文工具族（7 模块 .light 真实现 + 对拍测试）。
> 与既有缺陷不同，以下 3 项为本批 O0 真跑实测**新发现**，均以
> `_taskR11B_probe*.py` 探针在原生腿 O0 下复现；均为 workaround 可绕的
> codegen/runtime 行为缺陷，按红线不触碰 codegen 核心，归后续批次修复。

### 缺陷 R11B-1：`非` 对布尔假值判错（codegen）

- **现象**：`非 假` 应为 `真`，实际得 `假`；`非 假 == 真` 亦判假。
  `非 真`（→假）、`非 0`（整数 0 →真）、`非 5`（→假）正确。
- **探针证据**（O0 真跑）：`非 假`→`A=0`（错），`非 假 == 真`→`E=0`（错）。
- **根因**：`codegen_typed._gen_typed_unary_op` 的 `非` 实现为
  `dv_eq(operand, 整数 0)`，布尔假值 dv（bool 类型，值 0）与整数 0 dv
  跨类型比较，`dv_eq` 返回不等（0），`icmp ne 0,0` → false，判成「非假为假」。
- **影响**：所有对布尔变量取 `非` 的逻辑在原生腿 O0 下静默反向。
- **workaround**（R11B 已用）：一律用 `== 假` / 正逻辑替代 `非`（中文编码.light
  等模块已规避）。
- **✅ 已修复（R12A / 2026-09-06）**：`_gen_typed_unary_op` 的 `非` 改为
  `dv_to_bool` 取真值后 `icmp eq 0`（类型无关真值取反），布尔假值/函数返回值
  全部正确；中文编码/字符串工具/JSON核心 的 `== 假` workaround 已移除并回归
  全绿。定向测试 `test_codegen_unary_bool_O0.py::test_非_布尔字面量` 等。

### 缺陷 R11B-2：`整数(真)` 返回 0（bool→int 真 转换错误）

- **现象**：`整数(真)` 应返回 1，实际返回 0；`整数(假)` 返回 0（正确）。
- **探针证据**（O0 真跑）：`A=0`。
- **根因**：bool→int 转换对真值未映射为 1（与 Python `int(True)=1` 不一致）。
- **影响**：用 `整数(布尔)` 打印/累加布尔计数会静默得 0。
- **workaround**（R11B 已用）：需要 1/0 输出时用 `如果 … == 真: 输出(1) 否则: 输出(0)`。
- **✅ 已修复（R12A / 2026-09-06）**：`_gen_typed_builtin` 透传参数 AST
  （`arg_asts`），`整数` builtin 对静态类型 BOOL 的操作数直接从布尔字段
  （field 4）零扩展为 i64——`整数(真)`=1、`整数(假)`=0；类型未知操作数仍走
  `dv_to_int`（bool→int 的运行时映射归 T7）。定向测试
  `test_整数_布尔转换`（含布尔累加计数=2 对拍）。

### 缺陷 R11B-3：字典 `.获取` 方法不支持默认参数（返回 空）

- **现象**：`表.获取("k", "缺省")` 键存在亦返回 `空`；
  `表.获取("absent", "缺省")` 返回 `空` 而非缺省值。
- **探针证据**（O0 真跑）：`C=空`、`D=空`；顶层函数
  `字典获取(表, "absent", "缺省")`→`E=缺省`（workaround 有效）。
- **根因**：字典 `.获取` 方法调用的默认参数路径未生效（方法分派/参数传递）。
- **影响**：stdlib 中带默认值取字典值的 `.get` 形式在原生腿下静默返回 空。
- **workaround**（本批已用）：统一改用顶层 3 参 `字典获取(表, 键, 缺省)`
  （T7D 已注册 builtin）。
- **✅ 已修复（R12C / 2026-09-07）**：builtin `获取`/`get` 按接收者运行时
  类型分派（dict type 7 → dv_dict_get / dv_dict_get_def），字典方法形式
  带默认值正常；字符串/列表索引回落原路径。

## R11C：数据驱动 + 高级系统交互族真 .light 化（6 模块，2026-09-06）

### 概述

原生腿覆盖第 11 批 C：将 6 个数据驱动或高级系统交互模块从 .py 转写为纯光明
.light 实现，O0 下真编译真跑。数据驱动模块（农历/中国传统节日/中国行政区划）
嵌入数据表纯算法实现；高级系统交互模块（高级文件/图像处理基础/网络请求）基于
runtime 已有符号实现子集，无法实现的标注能力边界并提供降级实现。

### 模块清单与实现策略

| # | 模块 | 类型 | 实现策略 | 对拍状态 |
|---|---|---|---|---|
| 1 | 农历.light | 数据驱动 | 嵌入 2024-2030 农历数据表（月长/闰月/春节）+ 儒略日换算 + 干支生肖表 + 16 节日表 | 8 日期全对拍通过 |
| 2 | 中国传统节日.light | 数据驱动 | `从 农历 导入 农历转公历`，用精确农历数据计算节日公历日期；嵌入春节表+8农历节日+4公历节日 | 春节/公历节日与 .py 一致；农历节日用精确数据（.py 用简化估算，差 1-2 天，已标注） |
| 3 | 中国行政区划.light | 数据驱动 | 34 省数据（名称+2位代码+3-7主要城市）+ 9 省部分区县 + 42 城市代码；**数据通过分段函数返回**（6 组省级+3 组区县+3 组代码），规避 O0 临时槽位池溢出 | 省份/城市/区县/代码对拍通过 |
| 4 | 高级文件.light | 高级系统交互 | 复制文件（读+写）；目录大小（递归遍历+深度限制10层+跳过.和..）；文件树（递归文本生成）；磁盘使用/命令存在标注能力边界返回空/假 | 复制文件/磁盘使用/命令存在/目录大小（受控目录）通过 |
| 5 | 图像处理基础.light | 高级系统交互 | 纯数组像素表示（扁平列表 [r,g,b,...]），图像对象为字典；实现新建/读写像素/灰度化(0.299r+0.587g+0.114b)/反转/缩放(最近邻)/获取信息；打开/保存/滤镜/旋转标注能力边界抛明确错误 | 全部基础操作通过；能力边界返回明确错误 |
| 6 | 网络请求.light | 高级系统交互 | 基于 runtime socket 符号（B1）实现极简 HTTP/1.0 GET；URL 解析/编码/解码；响应解析；POST/HTTPS/重定向/chunked/超时标注能力边界返回 status=0 | URL 解析/编码/解码通过；POST/HTTPS 返回明确错误 |

### 新发现的 codegen/runtime 限制（本批踩坑登记）

1. **O0 临时槽位池上限 2048**：大型嵌套列表字面量（如完整行政区划表 ~400 城市）
   在模块初始化函数中会溢出。**workaround**：数据通过分段函数返回（每函数 ≤6 省），
   每函数独立槽位作用域。此为纯光明 workaround，未触碰 codegen 核心。
   **✅ 已修复（R12C / 2026-09-07）**：`_new_dv_slot` 不再硬性拒绝，动态抬高
   水位（alloca 行由 _emit_temp_slot_pool 按真实用量回填，无 IR 侧上限）；
   3000 元素字面量实测通过。区划分段函数保留为数据组织方式（功能等价）。

2. **字典访问返回列表拷贝**：`设 lst 为 d["key"]` 后修改 `lst[i]` 不影响
   `d["key"]`（值拷贝语义）；`设 d["key"][i] 为 val` 嵌套索引赋值也不生效。
   **workaround**：取出列表→修改→`设 d["key"] 为 lst` 写回字典。图像处理基础
   的 设置像素 已采用此模式。此为 runtime 字典值所有权架构限制（与 T5C-07 同源），
   归 T7 后续重构。

3. **try/catch 块内不能用 `跳过`(continue)**：会导致 LLVM IR 验证失败
   （基本块缺少终止指令）。**workaround**：用辅助函数 + 返回值替代循环内 continue。
   高级文件的 安全文件大小 已采用此模式。
   **✅ 已修复（R12C / 2026-09-07，实测核对）**：R12C 最小复现
   （当循环内 尝试/捕获 + 跳过）在当前 main 直接通过——此缺陷在 R11 登记后
   已被其它批次修复；高级文件 已恢复循环内 try/catch 自然写法，回归全绿。

4. **`_light_builtin.列出目录` 可能返回 "." 和 ".."**：递归目录遍历时必须跳过，
   否则无限递归导致段错误。高级文件的 目录大小/文件树 已加跳过逻辑。

5. **`输出("")` 不产生输出行**：空字符串输出被静默丢弃（无换行），对拍测试中
   需用 `长(字符串)` 等间接方式验证空返回值。

6. **目录递归在超大目录可能段错误**：即使跳过 "."/".."，项目根目录等超大目录树
   仍可能因内存/栈问题段错误。**workaround**：目录大小递归加深度限制 10 层；
   测试用受控小目录。此为能力边界，已在模块注释中声明。

### 能力边界清单（本批新增/确认）

| 模块 | 能力边界 | 降级行为 |
|---|---|---|
| 农历 | 数据范围 2024-2030（.py 为 1900-2100）；超出范围返回空/0 | 函数内判断年份范围，超出返回默认值 |
| 中国传统节日 | 农历节日用精确农历数据（.py 用简化估算，可能差 1-2 天）；节日倒计时无系统日期原语返回 -1 | 已在模块注释中注明差异 |
| 中国行政区划 | 每省仅保留 3-7 主要城市（非完整列表）；区县仅覆盖 9 省部分城市；区县级代码未嵌入 | 查询不存在的城市/区县返回空列表/空字符串 |
| 高级文件 | 磁盘使用情况/命令存在无 runtime 符号；目录大小在超大目录可能段错误；批量重命名/权限未实现 | 磁盘使用返回带"能力边界"标记的字典；命令存在返回假；目录大小深度限制 10 层 |
| 图像处理基础 | 无图像格式编解码（JPEG/PNG 等）；无高级滤镜/旋转/翻转/亮度/对比度/文本绘制/合并；仅支持 RGB/L 模式 | 打开/保存/滤镜等抛 运行时错误 明确声明；缩放用最近邻 |
| 网络请求 | 仅支持 HTTP GET；不支持 HTTPS/TLS、POST、重定向、chunked 编码、超时控制、非 ASCII URL 编码（用 %3F 占位） | POST/HTTPS 返回 status=0 的响应字典；非 ASCII 字符用 %3F 占位 |

### 验证（O0 真编译真跑，Windows）

- 新增 `tests/unit/test_原生腿_R11C_数据高级.py` 29 用例（6 模块，每模块 2-5 用例，
  能力边界模块验证降级路径）。
- **Windows**（clang 22.1.8 + MSVC 14.29 + SDK 10.0.19041）：**29 passed / 0 failed**
  （耗时 ~111 秒，O0 optimize_level=0）。
- 农历对拍 8 日期与 .py 完全一致（含 2025 闰六月、2024 元旦属上一农历年等边界）。
- 行政区划 34 省/主要城市/区县/代码与 .py 对拍通过。
- 图像处理 6 项基础操作（新建/读写像素/灰度化/反转/缩放/信息）+ 能力边界错误验证通过。
- 网络请求 URL 解析/编码/解码 + POST/HTTPS 能力边界验证通过。
- 高级文件 复制文件/磁盘使用/命令存在/目录大小（受控目录）通过。

### 仍存在的缺口

- 农历数据范围 2024-2030（.py 为 1900-2100），如需扩展需嵌入更多年份数据表
  （受 O0 槽位限制，可能需进一步分段或换编码方案）。
- 行政区划城市列表不完整（每省 3-7 主要城市），完整 ~400 城市列表受 O0 槽位限制
  无法嵌入。
- 字典值拷贝语义（T5C-07 同源）影响所有「字典内列表跨段变异」代码，需 runtime
  字典值所有权/引用计数重构（归 T7 后续）。
- 网络请求仅 HTTP GET，HTTPS/POST 等需 runtime TLS 支持（归后续批次）。
- 图像处理无文件 I/O，需 runtime 图像编解码支持（归后续批次）。
- 能力边界沿用（函数值/回调不可用等），无新增。
...
## R12A：一元运算符/布尔逻辑/类型转换族根因修复（2026-09-06）

### 概述

R11 批 4 个缺陷（R11A-01、R11A-07、R11B-1、R11B-2）的 codegen 根因修复，
并移除 R11 批对应 stdlib workaround。修复前 8 个最小复现全红 → 修复后全绿
→ R11A/B/C 回归 43 例全绿 → 能力清单重建后 11 passed。

### 修复内容

1. **`非`（R11A-01 / R11B-1）**：`_gen_typed_unary_op` 由
   `dv_eq(operand, 整数0)` 改为 `dv_to_bool` 真值 + `icmp eq 0`——类型无关
   真值取反，布尔假值、整数、字符串、函数返回值全正确。
2. **`整数(布尔)`（R11B-2）**：`_gen_typed_builtin` 加 `arg_asts` 参数
   （与 args 对齐的参数 AST，kwargs 重排时为 None），`整数` 对静态类型
   BOOL 的操作数从布尔字段（field 4）zext 为 i64。类型未知操作数仍走
   `dv_to_int`（运行时映射归 T7）。
3. **`且`/`或` 短路（R11A-07）**：两层修复——
   - **parser（根因）**：`_parse_logical_expr` 此前从未被表达式链调用，
     且/或 BinaryOp 节点根本不生成、右侧操作数被静默丢弃。现接入：
     `_parse_comparison` → `_parse_logical_or_expr`（或）→
     `_parse_logical_and_expr`（且/与）→ `_parse_comparison_chain`
     （原比较主体），优先级 或 < 且 < 比较（与 Python 一致）。
   - **codegen**：`_gen_typed_binary_op` 入口提前分派 且/或 至
     `_gen_short_circuit_logic`——左值 i8 槽默认存左真值，右臂块内覆盖，
     `br` 分支实现短路（and 左假/or 左真 跳过右臂），嵌套复合条件天然支持。
4. **能力清单**：codegen_typed.py 行号漂移 → `scripts/gen_native_capability_json.py`
   重建（builtin 397 / runtime 255，仅证据行号变化，无新增符号）。

### 移除的 workaround 清单

- `stdlib/中文编码.light`：8 处 `如果 _zhbmX(...) == 假:` → `如果 非 _zhbmX(...):`
  + 头部「不用非」注释改为修复标注。
- `stdlib/字符串工具.light`：4 处 `== 假` → `非`（含
  `非 是数字字符(字符) 且 非 是字母字符(字符)` 复合写法——且+非 组合验证）。
- `stdlib/JSON核心.light`：2 处 `己.是数字字符(...) == 假` → `非 己.是数字字符(...)`。
- 「整数(真) 的 if/else 打印」「拆嵌套如果」类 workaround 在 R11 stdlib 中
  无使用残留（grep 证实），仅存在于探针脚本（已随 _taskR11B_ 清理）。

### 验证证据

- 反跑：修复前 `test_codegen_unary_bool_O0.py` **8/8 红**
  （非恒假、且/或=左操作数、整数(布尔)恒0、短路右侧被求值）→ 修复后 **8/8 绿**。
- 回归：`test_原生腿_R11A_通用工具.py` + `test_原生腿_R11B_中文工具.py` +
...
## R13C：对拍覆盖扩展 + uuid v3/v5 真语义（2026-09-07）

### 对拍覆盖状态更新（R11A 章节第 3 条的后续）

R11A「每模块 3-5 个代表性用例」的覆盖缺口由本批补齐：
- **字符串工具**：+20 用例（大小写四件套/反转与长度/截取边界/分割连接/
  替换/空白四族/填充四件套/字符计数/子串查找族/包含开头结尾/字符串对齐/
  Base64 往返/URL 编码解码/分词四族/字符串相似度/空串边界族），
  oracle 直调 stdlib/字符串工具.py 同名实现。
- **数据结构**：+15 用例（栈 LIFO 序列与空边界/队列 FIFO/双端两端交错/
  优先队列乱序与同优先级稳定性/链表插删/查找获取修改/删除指定值/
  BST 插入查找/中序有序/高度边界/200 元素往返），oracle 以 Python 原生
  结构（list/堆语义/集合）镜像 .light 契约。
- 覆盖率：字符串工具 90 段中用例触及 ~55 函数（约 61%）；数据结构 76 段中
  触及 ~48 函数（约 63%）——达成 60% 目标。

### uuid v3/v5 能力边界更新（R11A 章节的降级标注撤销）

- runtime 已有二进制安全的 `dv_sha1`/`dv_md5`（显式长度）。
- 本批新增 `dv_uuid5`/`dv_uuid3`（runtime_typed.c）：16 原始命名空间字节 +
  名称 UTF-8 字节 → SHA1/MD5 → RFC 4122 版本位（3/5）+ 变体位 →
  8-4-4-4-12 小写。codegen 注册 `UUID5`/`UUID3` builtin；
  uuid工具.light 的 生成UUID5/生成UUID3 改调内建。
- 对拍：`test_uuid_v3v5_O0.py` 5 用例与 CPython `uuid.uuid5()/uuid.uuid3()`
  **逐字符一致**（DNS/URL/OID/X500 × 英文/中文/空名/200 字符名）。
  R11A 的「降级为确定性布局」标注撤销。

### 新发现的 bug（本批只登记，不修复）

| 编号 | 缺陷 | 复现 | 登记 |
|---|---|---|---|
| R13C-01 | `重复字符串(文本, 次数)` 的 `文本 乘以 次数` 在类型未知侧失效：dv_mul 对 str 返 0，输出 "0" | `从 字符串工具 导入 重复字符串` + `输出(重复字符串("ab", 3))` → `0` | T5C-01（浮点/未知侧类型推断）同族；规避：调用方保证字面量字符串或改用 重复字符 循环 |
| R13C-02 | `字符串对齐("hi", 6, "center")` 返回截断（右侧缺失，4 字符而非 6）；left/right 正常 | `字符串对齐("hi", 6, "center")` → `  hi` | 疑似分支内标量/返回值损坏（T6A-02 同族）；规避：直接用 居中填充（本批实测正确） |
| R13C-03 | `零填充(42, 5)` 数字参数时长(数字) 语义与 .py 的 str(数字) 不同（多补 0）；字符串参数正常 | `零填充(42, 5)` → `0000042` | 轻微口径差；规避：传字符串参数 |
| R13C-04 | oracle 侧：字符串工具.py 的 字符串相似度("", "") 除零（max(len)=0）；.light 实现有空串守卫返回 1.0 | Python 调用 `字符串相似度("", "")` | .py 侧缺陷，与原生腿无关 |
