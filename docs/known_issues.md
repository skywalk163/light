...
### 14.4 假测试基线在第四轮的两次变动（合并期已核，无需动作）
...
2. **`assertGreaterEqual` / `assertLessEqual` 未覆盖**：实测 34 处，
   与形态 2/5 同源。
3. **扫描根窄于形态的覆盖面**：CI 只扫 `tests/`，同形态违规在 `tests/` 之外
   还有 49 处匹配（按行去重约 47 条，主要在 `tools/`、`examples/` 的自测脚本里）。
   脚本本身支持 `--root .`，是接入口径限制了它。
...
按 1 + 2 全扩，基线会从 424 涨到约 700，与第四轮总纲「取保守口径、基线控制在
390 上下」的裁决冲突，故需先定口径。
