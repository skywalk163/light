...
| 日期 | 版本 | 变更 |
|------|------|------|
| 2026-09-08 10:06 | v6.0.0 | 初始版本，基于 v6.0 发布时的已知问题 |
...
| 编号 | 缺陷 | 最小复现 | 规避 |
|---|---|---|---|
| R11A-01 | **`非` 作用于函数调用失效**：`非 是数字字符("a")` 返回假（应为真）；对字面量/括号表达式正常 | `输出(非 是数字字符("a"))` | 改 `是数字字符(x) == 假` |
...
- **workaround**（本批已用）：一律用 `== 假` / 正逻辑替代 `非`（中文编码.light
  等模块已规避）。
...
- **workaround**（本批已用）：需要 1/0 输出时用 `如果 … == 真: 输出(1) 否则: 输出(0)`。
...
## R13B：mbedTLS bio_recv 握手自锁缺陷（2026-09-07）

### 缺陷 R13B-TLS：POSIX mbedTLS  bio_send 只入队不发送，bio_recv 阻塞读导致握手自锁

- **现象**：POSIX 下 HTTPS 握手永远阻塞（直到服务器超时断开 EOF）；Windows Schannel 正常。
- **根因**：`dv_tls_bio_send` 把 mbedTLS 加密产物（ClientHello 等）追加进 `out_pending` 队列但不直接写 socket；`dv_tls_bio_recv` 直接调用阻塞 `recv()` 等待服务器响应——但 ClientHello 从未上线，服务器不发送响应，死锁。Schannel 是纯状态机（发送与状态推进同步）故 Windows 正常。
