...
- **Workaround**：`格式化.light` 的 `文本去除缩进` 把「最小公共缩进」改放
  **单元素列表**（`最小盒`），min 比较改为分支内追加非负值。
- **归属**：codegen 循环 PHI/标量存活区间，归 T7。
...
- **缓解**：测试遵循 T5A 范式「每用例独立编译运行」，用例保持小型；
