...
### 一、本轮新增 codegen 缺陷（状态含 R12B 根因修复结果，2026-09-07 更新）

| 编号 | 缺陷 | 最小复现 | 规避 / 状态 |
| --- | --- | --- | --- |
| R11A-01 | **`非` 作用于函数调用失效**：`非 是数字字符("a")` 返回假（应为真）；对字面量/括号表达式正常 | `输出(非 是数字字符("a"))` | 改 `是数字字符(x) == 假`。⏳ 未修（归 R12A 一元/布尔） |
| R11A-02 | **内建函数名作局部变量 → 算术静默返 `[]`**：`删除`/`替换`/`插入`/`最小` 等（见 `src/keywords.py` 注册表）用作变量名后 `变量 加上 1` 得 `[]`，整条计算链报废 | `设 删除 为 1` → `删除 加上 1` | ✅ **已修复（R12B / 2026-09-07）**：调用名命中局部变量/形参表时按变量引用返回（局部变量遮蔽内建，Python 语义） |
| R11A-03 | **读列表下标 0 赋变量返 `[]`**：`设 x 为 dp[0]` → `[]`（dp[1]/dp[8] 正常） | `设 d 为 [1,2]` → `设 x 为 d[0]` | ✅ **main 已顺带修复**（R12B guard 用例 `test_R11A03_读下标0` 验证，未改代码） |
| R11A-04 | **`除以` 对 dict/list 读取值返 `空`**：真除路径对容器取出的值失效，`整除` 正常 | `设 v 为 字典获取(d,"k")` → `v 除以 2` | ✅ **已修复（R12B / 2026-09-07）**：runtime `dv_to_float` 补 REF 解引用（其余转换器早已 deref，唯独此处遗漏） |
| R11A-05 | **跨模块「导入即再导出」codegen 失败**：`从 正则 导入 验证邮箱` 的调用点报「未定义的段落」；同模块内再导出可用 | 跨模块原样再导出 | 薄包装 + 限定调用。⏳ 未修（归 R12C 方法分派/语法） |
| R11A-06 | **默认参数禁负数字面量**：`接收 甲 = -1` 报「意外的标记」（表达式内负值可用） | `段落 f 接收 甲 = -1:` | 用 `空` 哨兵。⏳ 未修 |
| R11A-07 | **`且`/`或` 内联条件解析失败且不短路** | `如果 a 且 b:` 复杂内联 | 拆嵌套 `如果`。⏳ 未修（归 R12A） |
| R11A-08 | **字符串内置方法返空**：`.去除空白()`/`.lstrip()`/`.rstrip()`/`.每个单词首字母大写()` 返 `[]`；`.分割()` 无参不做空白切分 | — | 手写循环。⏳ 未修（归 R12C 方法分派） |
| R11A-09 | **`转文本(列表)` 在 O0 打印为 `[]` 假象**：元素实际存在（列表句柄序列化缺陷） | `输出(转文本(列表))` | 对拍用 `长(列表)` + 下标逐元素打印。⏳ 未修 |
| R11A-10 | **嵌套容器不持久化**：list-in-list / dict-in-dict 的元素写入不落盘 | `设 d 为 字典获取(外,"k")` → 写 `d` | ✅ **已修复（R12B / 2026-09-07）**：codegen 沿 lvalue 链逐层写回 + runtime REF 写透（`dv_list_set_inplace`/`dv_set_ref`） |
| R11A-11 | **dict 跨函数写回不可靠**（T5C-07 函数内维度的同类）：函数内改 dict 不回传 | — | ✅ **已由 T7B REF 写回链覆盖**（R12B guard 用例 `test_R11A11_dict参数函数内修改写回` 验证，未改代码） |### 二、能力边界（降级实现，已在模块头部注释标注）
...
1. 未修条目（R11A-01/05/06/07/08/09）均为 **workaround 未根因修复**：R11A-01/07
   归 R12A（一元/布尔），R11A-05/08 归 R12C（方法分派/语法），R11A-06/09 待定。
   **已修条目见上表状态列（R12B / 2026-09-07）**。2. uuid v3/v5、断言 Callable 族、进度条实时刷新的真语义需语言层先补
...
2. **字典访问返回列表拷贝**：`设 lst 为 d["key"]` 后修改 `lst[i]` 不影响
   `d["key"]`（值拷贝语义）。**R12B 部分修复（2026-09-07）**：`dv_index_set`
   对 REF 句柄改为**写透真实存储**——`设 d 为 字典获取(外,"k")` → `设 d[i] 为 v`
   两段式现已穿透（REF 就地替换元素）；`设 d["key"][i] 为 val` 嵌套索引赋值
   亦由 codegen 多层链写回生效。**仍存（长期缺口）**：非写路径的语义级共享
   （如把 `d["key"]` 赋值给另一变量后分别 append 不互通）属 runtime 字典值
   所有权架构限制（与 T5C-07 同源），归 T7 后续重构。图像处理基础的 设置像素
   既有 workaround 无需回退。3. **try/catch 块内不能用 `跳过`(continue)**：会导致 LLVM IR 验证失败
